...
class ilObjExternalToolsSettingsGUI extends ilObjectGUI
{
	...
	*/
	...
	function editMathJaxObject()
	{
		...
		// Enable MathJax
// fau: mathJaxServer - rename checkbox for client-side rendering
...
// fau.
		...
		// mathjax limiter
		$options = array(
// fau: mathJaxServer - add invisible space to prevent a client-side rendering when chosen
			0 => '\&#8203;(...\&#8203;)',
// fau.
			1 => '[tex]...[/tex]',
...
// fau: mathJaxServer - add settings for server-side rendering
		include_once './Services/Utilities/classes/class.ilMathJax.php';
		$install_link = ' <a target="_blank" href="Services/Utilities/docs/Install-MathJax-Server.txt">'
		...
		$number_prop->setValue((int) $mathJaxSetting->get("server_timeout"));
...
// fau.
	...
	function saveMathJaxObject()
	{
		...
		if ($ilAccess->checkAccess("write", "", $this->object->getRefId()))
		{
// fau: mathJaxServer - save client aand server settings
			$mathJaxSetting = new ilSetting("MathJax");
			// Client settings
...
// fau: mathJaxServer - new function clearMathJaxCacheObject
	/**
