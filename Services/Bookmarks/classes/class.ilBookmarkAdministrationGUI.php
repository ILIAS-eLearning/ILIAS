...
class ilBookmarkAdministrationGUI
{
    ...
    */
    public function view()
    {
        ...
        $ilToolbar->addButton(
            $this->lng->txt("bookmark_new"),
            $this->ctrl->getLinkTarget($this, "newFormBookmark")
        );
        $ilToolbar->addButton(
            $this->lng->txt("bookmark_folder_new"),
     ...
     */
    public function newFormBookmarkFolder()
    {
        if (!$this->tree->isInTree($this->id)) {
            $this->ctrl->setParameter($this, 'bmf_id', '');
            $this->ctrl->redirect($this);
     ...
     */
    public function editFormBookmarkFolder()
    {
     ...
     */
    public function get_last($a_var, $a_value)
    {
     ...
     */
    public function editFormBookmark()
    {
        $lng = $this->lng;
        $ilCtrl = $this->ctrl;
     ...
     */
    public function createBookmarkFolder()
    {
        if (!$this->tree->isInTree($this->id)) {
            $this->ctrl->setParameter($this, 'bmf_id', '');
            $this->ctrl->redirect($this);
     ...
     */
    public function updateBookmarkFolder()
    {
        if (!$this->tree->isInTree($_GET["obj_id"])) {
            $this->ctrl->setParameter($this, 'obj_id', '');
            $this->ctrl->setParameter($this, 'bmf_id', '');
     ...
     * export bookmarks
     */
    public function export($deliver = true)
    {
        $ilErr = $this->error;
        $ilUser = $this->user;
        ...
        $html_content = ilBookmarkImportExport::_exportBookmark(
            $export_ids,
            true,
