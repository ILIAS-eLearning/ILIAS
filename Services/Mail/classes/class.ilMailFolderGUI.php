...
class ilMailFolderGUI
{
    ...
    */
    ...
    {
        if (isset($_POST["subfolder_title"]) && 'tree' == ilSession::get(ilMailGUI::VIEWMODE_SESSION_KEY)) {
            $_SESSION["subfolder_title"] = ilUtil::stripSlashes($_POST['subfolder_title']);
        }

        if (empty($_POST['subfolder_title'])) {
            ilUtil::sendInfo($this->lng->txt("mail_insert_folder_name"));
            return $this->addSubFolder();
        } elseif ($mobj_id = $this->mbox->addFolder($_GET["mobj_id"], ilUtil::stripSlashes($_POST["subfolder_title"]))) {
    ...
    {
        if (isset($_POST["subfolder_title"]) && 'tree' == ilSession::get(ilMailGUI::VIEWMODE_SESSION_KEY)) {
            $_SESSION["subfolder_title"] = $_POST['subfolder_title'];
        }
        ...
        switch ($this->current_selected_cmd) {
            ...
            case 'deleteMails':
                ...
                if ($this->mbox->getTrashFolder() == $_GET["mobj_id"]) {
                    ...
                    if (!is_array($_POST["mail_id"])) {
                        ilUtil::sendInfo($this->lng->txt("mail_select_one"));
                    } elseif ($this->umail->moveMailsToFolder($_POST["mail_id"], $this->mbox->getTrashFolder())) {
                        $_GET["offset"] = 0;
            ...
            default:
                if (!is_array($_POST["mail_id"])) {
                    ilUtil::sendInfo($this->lng->txt("mail_select_one"));
                } elseif ($this->umail->moveMailsToFolder($_POST["mail_id"], $this->current_selected_cmd)) {
    ...
    public function confirmDeleteMails()
    {
        if ($this->mbox->getTrashFolder() == $_GET['mobj_id']) {
            ...
            if (!is_array($mail_ids)) {
                ilUtil::sendInfo($this->lng->txt('mail_select_one'));
            } elseif ($this->umail->deleteMails($mail_ids)) {
                $_GET['offset'] = 0;
                ilUtil::sendInfo($this->lng->txt('mail_deleted'));
            } else {
