...
use ILIAS\App\CoreApp\Course\Infrastructure\Repository\Doctrine\CourseMemberEntityRepository;
use \ILIAS\App\CoreApp\Course\Domain\Entity\CourseMember;

use Symfony\Component\Messenger\MessageBus;
use ILIAS\App\CoreApp\Member\Infrastructure\Repository\Doctrine\MemberEntityRepository;
use ILIAS\App\Infrasctrutre\Persistence\Doctrine\DoctrineEntityManager;
use ILIAS\App\Infrasctrutre\CommandHandler\CommandHandlerMessageMiddleware;
use ILIAS\App\CoreApp\Member\Domain\Service\MemberWriteonlyService;
use ILIAS\App\CoreApp\Member\Domain\Command\HandlersLocator;
...
class ilMembershipGUI {
	...
	public function addUserFromAutoComplete() {
		...
		$member_entity_repository = new MemberEntityRepository(DoctrineEntityManager::getInstance());

		$bus = new MessageBus([
			new CommandHandlerMessageMiddleware(new HandlersLocator($member_entity_repository->getWriteonlyRepository()))
		]);

		$course_service = new MemberWriteonlyService($bus);
		$course_service->addMember(ilObject::_lookupObjectId($_GET['ref_id']), 292);
		$course_service->addMember(ilObject::_lookupObjectId($_GET['ref_id']), 6);

	}


	public function membersSrcDoctrine() {
		global $DIC;
		$ilToolbar = $DIC['ilToolbar'];

		if ($this->canAddOrSearchUsers()) {
			include_once './Services/Search/classes/class.ilRepositorySearchGUI.php';
			ilRepositorySearchGUI::fillAutoCompleteToolbar($this, $ilToolbar, array(
			...
			// search button
			$ilToolbar->addButton($this->lng->txt($this->getParentObject()->getType()
				. "_search_users"), $this->ctrl->getLinkTargetByClass('ilRepositorySearchGUI', 'start'));

			// separator
			$ilToolbar->addSeparator();

			///
			$course_member_entity_repository = new CourseMemberEntityRepository(DoctrineEntityManager::getInstance());
			$members = $course_member_entity_repository->getReadonlyRepository()->findAllByObjId(ilObject::_lookupObjectId($_GET['ref_id']));


			$arr_member = array_map(function (CourseMember $member): string {
				return "<tr><td>".$member->getUser()->getLastname()."</td>
			...
			$this->tpl->setVariable('MEMBERS',  "<table>".implode("",$arr_member )."</table>");
		}
	}
	...
	public function executeCommand() {
		...
		switch ($next_class) {
			...
			case 'ilrepositorysearchgui':
				...
				if ($participants->isAdmin($GLOBALS['DIC']['ilUser']->getId())
					|| $ilAccess->checkAccess('manage_members', '', $this->getParentObject()->getRefId())) {
					$rep_search->setCallback($this, 'assignMembers', $this->getParentGUI()->getLocalRoles());
				} else {
					//#18445 excludes admin role
					$rep_search->setCallback($this, 'assignMembers', $this->getLocalRoles(array( $this->getParentObject()->getDefaultAdminRole() )));
				}
			...
			case 'ilmailmembersearchgui':
				$ilTabs->clearTargets();
				$ilTabs->setBackTarget($this->lng->txt('btn_back'), $this->ctrl->getLinkTarget($this, $this->getDefaultCommand()));

				$mail = new ilMail($ilUser->getId());
				if (!($this->getParentObject()->getMailToMembersType() == ilCourseConstants::MAIL_ALLOWED_ALL
						|| $ilAccess->checkAccess('manage_members', "", $this->getParentObject()->getRefId()))
					|| !$rbacsystem->checkAccess('internal_mail', $mail->getMailObjectReferenceId())) {
					$ilErr->raiseError($this->lng->txt("msg_no_perm_read"), $ilErr->MESSAGE);
				}

				$mail_search = new ilMailMemberSearchGUI($this, $this->getParentObject()->getRefId(), $this->getMailMemberRoles());
				$mail_search->setObjParticipants(ilParticipants::getInstance($this->getParentObject()->getRefId()));
			...
			case 'ilusersgallerygui':
				...
				$tabs = $GLOBALS['DIC']->tabs()->setSubTabActive($this->getParentObject()->getType() . '_members_gallery');
				...
				if (!$is_admin && ($this->getParentObject()->getShowMembers() == 0 || !$is_participant)) {
					$ilErr->raiseError($this->lng->txt('msg_no_perm_read'), $ilErr->MESSAGE);
				}
			...
			default:
				...
				if ($cmd == "mailMembersBtn" || $cmd == 'membersMap' || $cmd == 'printForMembersOutput' || $cmd == 'jump2UsersGallery') {
	...
	public function updateParticipants() {
		...
		$hasEditPermissionAccess = ($ilAccess->checkAccess('edit_permission', '', $this->getParentObject()->getRefId()) or $this->getMembersObject()
				->isAdmin($ilUser->getId()));
		...
		foreach ($_POST['participants'] as $usr_id) {
			...
			if (!$hasEditPermissionAccess && $memberIsAdmin && !in_array($adminRoleId, $_POST['roles'][$usr_id])) {
			...
			foreach ((array)$_POST['roles'][$usr_id] as $role_id) {
				...
				if (!$hasEditPermissionAccess && $role_id == $adminRoleId && !$memberIsAdmin) {
		...
		foreach ($_POST['participants'] as $usr_id) {
			...
			if ($this instanceof ilCourseMembershipGUI) {
				...
				$this->getMembersObject()->sendNotification($this->getMembersObject()->NOTIFY_STATUS_CHANGED, $usr_id);
			}

			if (($this->getMembersObject()->isAdmin($usr_id) || $this->getMembersObject()->isTutor($usr_id)) && in_array($usr_id, $contact)) {
	...
	protected function confirmDeleteParticipants() {
		...
		if (!$ilAccess->checkAccess('edit_permission', '', $this->getParentObject()->getRefId())
		...
		foreach ($participants as $usr_id) {
			...
			$confirm->addItem('participants[]', $name['user_id'], $name['lastname'] . ', ' . $name['firstname'] . ' [' . $name['login']
	...
	protected function sendMailToSelectedUsers() {
		...
		ilUtil::redirect(ilMailFormCall::getRedirectTarget($this, 'participants', array(), array(
				'type' => 'new',
	...
	protected function membersMap() {
		...
		$map->setMapId("course_map")->setWidth("700px")->setHeight("500px")->setLatitude($this->getParentObject()->getLatitude())
			->setLongitude($this->getParentObject()->getLongitude())->setZoom($this->getParentObject()->getLocationZoom())->setEnableTypeControl(true)
			->setEnableNavigationControl(true)->setEnableCentralMarker(true);
	...
	protected function showParticipantsToolbar() {
		...
		// print button
		$ilToolbar->addButton($this->lng->txt($this->getParentObject()->getType()
			. "_print_list"), $this->ctrl->getLinkTarget($this, 'printMembers'));
	...
	protected function showMemberExportToolbarButton(ilToolbarGUI $toolbar, $a_back_cmd = null, $a_separator = false) {
		if ($this->getParentObject()->getType() == 'crs' && $this->getParentObject()->getShowMembersExport()) {
			if ($a_separator) {
			...
			$toolbar->addButton($this->lng->txt($this->getParentObject()->getType()
				. '_print_list'), $this->ctrl->getLinkTarget($this, 'printForMembersOutput'));
		}
	}
	...
	protected function showMailToMemberToolbarButton(ilToolbarGUI $toolbar, $a_back_cmd = null, $a_separator = false) {
		...
		if (($this->getParentObject()->getMailToMembersType() == 1)
			|| ($ilAccess->checkAccess('manage_members', "", $this->getParentObject()->getRefId())
				&& $rbacsystem->checkAccess('internal_mail', $mail->getMailObjectReferenceId()))) {
			...
			$toolbar->addButton($this->lng->txt("mail_members"), $this->ctrl->getLinkTargetByClass('ilMailMemberSearchGUI', ''));
		}
	}
	...
	protected function getDefaultCommand() {
		$has_manage_members_permission = $this->checkRbacOrPositionAccessBool('manage_members', 'manage_members', $this->getParentObject()
			->getRefId());
	...
	public function addMemberTab(ilTabsGUI $tabs, $a_is_participant = false) {
		...
		if ($has_manage_members_permission) {
			$tabs->addTab('members', $member_tab_name, $this->ctrl->getLinkTarget($this, ''));
		} elseif ((bool)$this->getParentObject()->getShowMembers() && $a_is_participant) {
			$tabs->addTab('members', $member_tab_name, $this->ctrl->getLinkTargetByClass(array( get_class($this), 'ilusersgallerygui' ), 'view'));
		} elseif ($this->getParentObject()->getMailToMembersType() == 1
			&& $GLOBALS['DIC']['rbacsystem']->checkAccess('internal_mail', $mail->getMailObjectReferenceId())
			&& $a_is_participant) {
			$tabs->addTab('members', $member_tab_name, $this->ctrl->getLinkTarget($this, "mailMembersBtn"));
		}
	}
	...
	protected function setSubTabs(ilTabsGUI $tabs) {
		...
		if ($this->checkRbacOrPositionAccessBool('manage_members', 'manage_members', $this->getParentObject()->getRefId())) {
			$tabs->addSubTabTarget($this->getParentObject()->getType()
				. "_member_administration", $this->ctrl->getLinkTarget($this, 'participants'), "members", get_class($this));

			// show group overview
			if ($this instanceof ilCourseMembershipGUI) {
				$tabs->addSubTabTarget("crs_members_groups", $this->ctrl->getLinkTargetByClass("ilCourseParticipantsGroupsGUI", "show"), "", "ilCourseParticipantsGroupsGUI");
			}

			$childs = (array)$GLOBALS['DIC']['tree']->getChildsByType($this->getParentObject()->getRefId(), 'sess');
			if (count($childs)) {
				$tabs->addSubTabTarget('events', $this->ctrl->getLinkTargetByClass(array(
					get_class($this),
					'ilsessionoverviewgui'
				), 'listSessions'), '', 'ilsessionoverviewgui');
			}

			$tabs->addSubTabTarget($this->getParentObject()->getType()
				. '_members_gallery', $this->ctrl->getLinkTargetByClass(array( get_class($this), 'ilUsersGalleryGUI' )), 'view', 'ilUsersGalleryGUI');
		} else {
			if ($this->getParentObject()->getShowMembers()) {
				// gallery
				$tabs->addSubTabTarget($this->getParentObject()->getType() . '_members_gallery', $this->ctrl->getLinkTargetByClass(array(
					get_class($this),
					'ilUsersGalleryGUI'
				)), 'view', 'ilUsersGalleryGUI');
			}
		}
		...
		if (ilMapUtil::isActivated() && $this->getParentObject()->getEnableMap()) {
			$tabs->addSubTabTarget($this->getParentObject()->getType()
				. '_members_map', $this->ctrl->getLinkTarget($this, 'membersMap'), "membersMap", get_class($this));
		}

		include_once 'Services/PrivacySecurity/classes/class.ilPrivacySettings.php';
		if (ilPrivacySettings::_getInstance()->checkExportAccess($this->getParentObject()->getRefId())) {
			$tabs->addSubTabTarget('export_members', $this->ctrl->getLinkTargetByClass(array(
				get_class($this),
				'ilmemberexportgui'
			), 'show'), '', 'ilmemberexportgui');
		}
	}
	...
	protected function parseSubscriberTable() {
		...
		$subscriber->readSubscriberData($filtered_subscribers);
	...
	public function confirmAssignSubscribers() {
		...
		foreach ($_POST["subscribers"] as $subscribers) {
			...
			$c_gui->addItem('subscribers[]', $name['user_id'], $name['lastname'] . ', ' . $name['firstname'] . ' [' . $name['login']
				. ']', ilUtil::getImagePath('icon_usr.svg'));
		}
		...
		if (!$this->getMembersObject()->deleteSubscribers($_POST["subscribers"])) {
			...
			foreach ($_POST['subscribers'] as $usr_id) {
				...
				if ($this instanceof ilGroupMembershipGUI) {
					include_once './Modules/Group/classes/class.ilGroupMembershipMailNotification.php';
					$this->getMembersObject()->sendNotification(ilGroupMembershipMailNotification::TYPE_REFUSED_SUBSCRIPTION_MEMBER, $usr_id);
				}
				...
				if ($this instanceof ilLearningSequenceMembershipGUI) {
					$this->getMembersObject()
						->sendNotification(ilLearningSequenceMembershipMailNotification::TYPE_REFUSED_SUBSCRIPTION_MEMBER, $usr_id);
				}
			}
		}
	...
	public function assignSubscribers() {
		...
		if (!$this->getMembersObject()->assignSubscribers($_POST["subscribers"])) {
			...
			foreach ($_POST["subscribers"] as $usr_id) {
				...
				if ($this instanceof ilGroupMembershipGUI) {
					include_once './Modules/Group/classes/class.ilGroupMembershipMailNotification.php';
					$this->getMembersObject()->sendNotification(ilGroupMembershipMailNotification::TYPE_ACCEPTED_SUBSCRIPTION_MEMBER, $usr_id);
				}
				if ($this instanceof ilSessionMembershipGUI) {
	...
	protected function parseWaitingListTable() {
		...
		$waiting_table->setUserIds($wait_users);
	...
	public function confirmAssignFromWaitingList() {
		...
		foreach ($_POST["waiting"] as $waiting) {
			...
			$c_gui->addItem('waiting[]', $name['user_id'], $name['lastname'] . ', ' . $name['firstname'] . ' [' . $name['login']
				. ']', ilUtil::getImagePath('icon_usr.svg'));
		}
	...
	protected function refuseFromList() {
		...
		foreach ($_POST["waiting"] as $user_id) {
			...
			if ($this instanceof ilGroupMembershipGUI) {
				include_once './Modules/Group/classes/class.ilGroupMembershipMailNotification.php';
				$this->getMembersObject()->sendNotification(ilGroupMembershipMailNotification::TYPE_REFUSED_SUBSCRIPTION_MEMBER, $user_id, true);
			}
			if ($this instanceof ilSessionMembershipGUI) {
				include_once './Modules/Session/classes/class.ilSessionMembershipMailNotification.php';
				$noti = new ilSessionMembershipMailNotification();
	...
	protected function printMembersOutput() {
		...
		$this->member_data = $this->getPrintMemberData($this->filterUserIdsByRbacOrPositionOfCurrentUser($this->getMembersObject()
			->getParticipants()));
	...
	protected function initAttendanceList($a_for_members = false) {
		...
		if ($this instanceof ilSessionMembershipGUI) {
			$member_id = $DIC->repositoryTree()->checkForParentType($this->getParentObject()->getRefId(), 'grp');
			if (!$member_id) {
				$member_id = $DIC->repositoryTree()->checkForParentType($this->getParentObject()->getRefId(), 'crs');
			}
			if (!$member_id) {
			...
			$list = new ilAttendanceList($this, $this->getParentObject(), $part, $waiting_list);
		} else {
			include_once 'Services/Membership/classes/class.ilAttendanceList.php';
			$list = new ilAttendanceList($this, $this->getParentObject(), $this->getMembersObject(), $waiting_list);
		}
		$list->setId($this->getParentObject()->getType() . '_memlist_' . $this->getParentObject()->getId());

		$list->setTitle($this->lng->txt($this->getParentObject()->getType() . '_members_print_title'), $this->lng->txt('obj_'
				. $this->getParentObject()->getType()) . ': ' . $this->getParentObject()->getTitle());

		include_once './Services/Tracking/classes/class.ilObjUserTracking.php';
		$show_tracking = (ilObjUserTracking::_enabledLearningProgress() and ilObjUserTracking::_enabledUserRelatedData());
		if ($show_tracking) {
