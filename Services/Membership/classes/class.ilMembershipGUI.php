...
use ILIAS\Modules\Course\Domain\Repository\CourseMemberRepository;
use ILIAS\Modules\Course\Infrastructure\Repository\Doctrine\CourseMemberEntityRepository;
...
class ilMembershipGUI {
	...
	protected function participants() {
		...
		require_once "./Customizing/demo/bootstrap.php";
		require_once "./src/Modules/Course/Domain/Repository/CourseMemberRepository.php";

		$crs_member_entity_repository = new CourseMemberEntityRepository($entityManager);

		$crs_member_repository = new CourseMemberRepository($crs_member_entity_repository);

		$tests = $crs_member_repository->findallByObjId(282);
...
/*
		...
		foreach ($tests as $object) {
			...
			$usr .= $object->getUser()->getLastname();
			/*print_r($object);exit;
			echo sprintf("-%s\n", $object->getTitle());*/
		}
		
		$this->tpl->setVariable('MEMBERS', $usr);
	}
