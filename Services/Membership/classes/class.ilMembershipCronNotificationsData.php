...
class ilMembershipCronNotificationsData
{
	 ...
	 * @param
	 */
	function __construct($last_run, $cron_id)
	{
		global $DIC;
	...
	protected function load()
	{
		...
		if(sizeof($this->objects))
		{
			...
			foreach ($this->objects as $ref_id => $user_ids)
			{
				...
				if ($news_item->checkNewsExistsForObjects($objs["obj_id"], $this->last_run))
				{
					...
					foreach ($user_ids as $user_id)
					{
						...
						$user_news = $news_item->getNewsForRefId($ref_id,
							false, false, $this->last_run, false, false, false, false,
							$user_id);
						if ($user_news)
						{
				...
				foreach (array_keys($objs["obj_id"]) as $obj_id)
				{
					$coms = ilNote::_getAllNotesOfSingleRepObject($obj_id, IL_NOTE_PUBLIC, false, false,
						$this->last_run);
					foreach ($coms as $c)
					{
						if ($c->getNewsId() == 0)	{
