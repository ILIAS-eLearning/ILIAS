...
class ilTemplate extends HTML_Template_ITX {
	 ...
	 * @access    public
	 */
	function __construct($file, $flag1, $flag2, $in_module = false, $vars = "DEFAULT", $plugin = false, $a_use_cache = true) {
		global $DIC;
	...
	*/
	 ...
	 * @return    string
	 */
	function get($part = "DEFAULT", $add_error_mess = false, $handle_referer = false, $add_ilias_footer = false, $add_standard_elements = false, $a_main_menu = true, $a_tabs = true) {
		global $DIC;
		...
		foreach ($pl_names as $pl) {
			...
			$resp = $gui_class->getHTML("", "template_get", array( "tpl_id" => $this->tplIdentifier, "tpl_obj" => $this, "html" => $html ));
	...
	function fillWindowTitle() {
		...
		if ($ilSetting->get('short_inst_name') != "") {
			$this->setVariable("WINDOW_TITLE", $ilSetting->get('short_inst_name'));
		} else {
			$this->setVariable("WINDOW_TITLE", "ILIAS");
		}
	}
	...
	public function fillCssFiles($a_force = false) {
		...
		foreach ($this->css_files as $css) {
			...
			if (is_file($filename) || substr($filename, 0, 4) == "http" || substr($filename, 0, 2) == "//" || $a_force) {
	...
	function fillContentStyle() {
		if ($this->content_style_sheet != "") {
			...
			$this->setVariable("LOCATION_CONTENT_STYLESHEET", $this->content_style_sheet);
			$this->parseCurrentBlock();
		}
	}
	...
	private function fillNewContentStyle() {
		$this->setVariable("LOCATION_NEWCONTENT_STYLESHEET_TAG", '<link rel="stylesheet" type="text/css" href="'
	...
	function addILIASFooter() {
		...
		if (DEVMODE) {
			if (function_exists("tidy_parse_string")) {
				...
				$link_items[ilUtil::appendUrlParameterString($_SERVER["REQUEST_URI"], "do_dev_validate=accessibility")] = array(
					"Accessibility",
					true
				);
			}
	...
	function handleReferer() {
		if (((substr(strrchr($_SERVER["PHP_SELF"], "/"), 1) != "error.php")
			...
			if (isset($url_parts["query"]) && preg_match("/cmd=gateway/", $url_parts["query"]) && (isset($_POST["cmd"]["create"]))) {
				...
				if (isset($url_parts["query"]) && preg_match("/cmd=post/", $url_parts["query"]) && (isset($_POST["cmd"]["create"]))) {
					foreach ($_POST as $key => $val) {
						if (is_array($val)) {
							$val = key($val);
						}
	...
	function addBlockFile($var, $block, $tplname, $in_module = false) {
		...
		foreach ($pl_names as $pl) {
			...
			$resp = $gui_class->getHTML("", "template_add", array( "tpl_id" => $id, "tpl_obj" => $this, "html" => $template ));
	...
	function getTemplatePath($a_tplname, $a_in_module = false, $a_plugin = false) {
		...
		if (strpos($a_tplname, "/") === false) {
			...
			if (ilStyleDefinition::getCurrentSkin() != "default") {
				...
				$fname = "./Customizing/global/skin/" . ilStyleDefinition::getCurrentSkin() . "/" . $style . "/" . $module_path
					. basename($a_tplname);
			...
			if (strpos($a_tplname, "src/UI") === 0) {
				if (class_exists("ilStyleDefinition") // for testing
					...
					$fname = "./Customizing/global/skin/" . ilStyleDefinition::getCurrentSkin() . "/"
						. str_replace("src/UI/templates/default", "UI", $a_tplname);
				}
				if ($fname == "" || !file_exists($fname)) {
					$fname = $a_tplname;
				}
	...
	private function fillLeftContent() {
		if (trim($this->left_content) != "") {
			...
			$left_col_class = (trim($this->right_content) == "") ? "col-sm-3 col-sm-pull-9" : "col-sm-3 col-sm-pull-6";
	...
	function fillSideIcons() {
		...
		if ($this->tree_flat_link != "") {
			...
			$this->setVariable("TREE_ACC_KEY", ilAccessKeyGUI::getAttribute(($this->tree_flat_mode
				== "tree") ? ilAccessKey::TREE_ON : ilAccessKey::TREE_OFF));
			$this->parseCurrentBlock();
		}
	 ...
	 * @param bool   $a_add_version_parameter
	 * @param int    $a_batch
	 */
	public function addJavaScript($a_js_file, $a_add_version_parameter = true, $a_batch = 2) {
		...
		if (is_int(strpos($a_js_file, "Services/jQuery")) || is_int(strpos($a_js_file, "/jquery.js"))
	...
	function setEditPageButton($a_link, $a_txt, $a_frame) {
		$this->edit_page_button = array( "link" => $a_link, "txt" => $a_txt, "frame" => $a_frame );
	}
	...
	function addAdminPanelCommand($a_cmd, $a_txt, $a_arrow = false) {
		$this->admin_panel_commands[] = array( "cmd" => $a_cmd, "txt" => $a_txt );
		if ($a_arrow) {
	...
	function fillPermanentLink() {
		if (is_array($this->permanent_link)) {
			...
			$plinkgui = new ilPermanentLinkGUI($this->permanent_link["type"], $this->permanent_link["id"], $this->permanent_link["append"], $this->permanent_link["target"]);
