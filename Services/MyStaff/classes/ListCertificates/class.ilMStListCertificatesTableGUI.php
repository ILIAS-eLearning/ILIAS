...
class ilMStListCertificatesTableGUI extends ilTable2GUI {
	 ...
	 * @return array
	 */
	protected function getFieldValuesForExport(ilMStListCertificate $my_staff_course) {
		...
		foreach ($this->getSelectedColumns() as $k => $v) {
			switch ($k) {
				case 'usr_assinged_orgus':
					...
					break;
				case 'usr_reg_status':
					$field_values[$k] = ilMStListCertificate::getMembershipStatusText($my_staff_course->getUsrRegStatus());
					break;
				case 'usr_lp_status':
