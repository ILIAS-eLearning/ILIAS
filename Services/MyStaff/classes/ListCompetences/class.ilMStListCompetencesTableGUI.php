 ...
 */
class ilMStListCompetencesTableGUI extends ilTable2GUI {
	...
	/**
	 * @param ilMStListCompetencesGUI $parent_obj
	 * @param string              $parent_cmd
	 */
	public function __construct(ilMStListCompetencesGUI $parent_obj, $parent_cmd = ilMStListCompetencesGUI::CMD_INDEX) {
		global $DIC;
	...
	protected function parseData() {
		...
		$count = ilMStListCompetences::getData($all_users_for_user, $options);
		$options['limit'] = array(
			'start' => intval($this->getOffset()),
			'end' => intval($this->getLimit()),
		);
		$options['count'] = false;
		$data = ilMStListCompetences::getData($all_users_for_user, $options);
	...
	public function initFilter() {
		...
		$item->setOptions(array(
			...
			ilMStListCompetence::MEMBERSHIP_STATUS_REQUESTED => $DIC->language()->txt('mst_memb_status_requested'),
			ilMStListCompetence::MEMBERSHIP_STATUS_WAITINGLIST => $DIC->language()->txt('mst_memb_status_waitinglist'),
			ilMStListCompetence::MEMBERSHIP_STATUS_REGISTERED => $DIC->language()->txt('mst_memb_status_registered'),
		));
		$this->addFilterItem($item);
		$item->readFromSession();
		$this->filter["memb_status"] = $item->getValue();

		if (ilObjUserTracking::_enabledLearningProgress() && $this->access->hasCurrentUserAccessToCompetenceLearningProgressForAtLeastOneUser()) {
	...
	public function getSelectableColumns() {
		...
		$cols['comp_title'] = array(
			'txt' => $DIC->language()->txt('comp_title'),
			'default' => true,
			'width' => 'auto',
			'sort_field' => 'crs_title',
		);
		$cols['comp_level'] = array(
			'txt' => $DIC->language()->txt('comp_level'),
			'default' => true,
			'width' => 'auto',
	...
	/**
	 * @param ilMStListCompetence $my_staff_course
	 */
	public function fillRow($my_staff_course) {
		...
		foreach ($this->getSelectableColumns() as $k => $v) {
			if ($this->isColumnSelected($k)) {
				switch ($k) {
					...
					case 'usr_reg_status':
						$this->tpl->setCurrentBlock('td');
						$this->tpl->setVariable('VALUE', ilMStListCompetence::getMembershipStatusText($my_staff_course->getUsrRegStatus()));
						$this->tpl->parseCurrentBlock();
						break;
					case 'usr_lp_status':
						$this->tpl->setCurrentBlock('td');
		...
		$DIC->ctrl()->setParameterByClass(ilMStListCompetencesGUI::class, 'mst_lco_usr_id', $my_staff_course->getUsrId());
		$DIC->ctrl()->setParameterByClass(ilMStListCompetencesGUI::class, 'mst_lco_crs_ref_id', $my_staff_course->getCrsRefId());
	 ...
	 * @return array
	 */
	protected function getFieldValuesForExport(ilMStListCompetence $my_staff_course) {
		$propGetter = Closure::bind(function ($prop) { return $this->$prop; }, $my_staff_course, $my_staff_course);
		...
		foreach ($this->getSelectedColumns() as $k => $v) {
			switch ($k) {
				case 'usr_assinged_orgus':
					...
					break;
				case 'usr_reg_status':
					$field_values[$k] = ilMStListCompetence::getMembershipStatusText($my_staff_course->getUsrRegStatus());
					break;
				case 'usr_lp_status':
