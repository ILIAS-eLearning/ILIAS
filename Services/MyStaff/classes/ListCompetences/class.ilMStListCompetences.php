...
class ilMStListCompetences {
	...
	public static function getData(array $arr_usr_ids = array(), array $options = array()) {
		...
		$select = 'SELECT crs_ref.ref_id AS crs_ref_id, crs.title AS crs_title, reg_status, lp_status, usr_data.usr_id AS usr_id, usr_data.login AS usr_login, usr_data.lastname AS usr_lastname, usr_data.firstname AS usr_firstname, usr_data.email AS usr_email  FROM (
	                    SELECT reg.obj_id, reg.usr_id, ' . ilMStListCompetence::MEMBERSHIP_STATUS_REGISTERED . ' AS reg_status, lp.status AS lp_status FROM obj_members 
		          AS reg
		            ...
		            UNION
	                    SELECT obj_id, usr_id, ' . ilMStListCompetence::MEMBERSHIP_STATUS_WAITINGLIST . ' AS reg_status, 0 AS lp_status FROM crs_waiting_list AS waiting
	                    WHERE ' . $DIC->database()->in('waiting.usr_id', $arr_usr_ids, false, 'integer') . '
                    UNION
	                    SELECT obj_id, usr_id, ' . ilMStListCompetence::MEMBERSHIP_STATUS_REQUESTED . ' AS reg_status, 0 AS lp_status FROM il_subscribers AS requested
		...
		while ($crs = $DIC->database()->fetchAssoc($result)) {
			$list_course = new ilMStListCompetence();
