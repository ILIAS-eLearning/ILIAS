    ...
    public function executeCommand()
    {
        ...
        switch ($next_class) {
            case "illearningprogressgui":
                ...
                $new_gui = new ilLearningProgressGUI(ilLearningProgressGUI::LP_CONTEXT_USER_FOLDER, USER_FOLDER_ID,
                    $this->object->getId());
    ...
    */
    public function setTitleAndDescription()
    {
        if (strtolower(get_class($this->object)) == 'ilobjuser') {
            ...
            $this->tpl->setTitleIcon(ilUtil::getImagePath("icon_" . $this->object->getType() . ".svg"),
    ...
    public function initCreate()
    {
        ...
        foreach ($obj_list as $obj_data) {
            ...
            if ($this->object->getRefId() != USER_FOLDER_ID and !in_array(SYSTEM_ROLE_ID,
            ...
            if ($obj_data["obj_id"] != ANONYMOUS_ROLE_ID) {
                ...
                if ($obj_data["obj_id"] != SYSTEM_ROLE_ID or in_array(SYSTEM_ROLE_ID,
                        $rbacreview->assignedRoles($ilUser->getId()))) {
        ...
        if (!count($rol)) {
            $this->ilias->raiseError($this->lng->txt("msg_no_roles_users_can_be_assigned_to"),
    ...
    public function editObject()
    {
        ...
        if ($this->usrf_ref_id == USER_FOLDER_ID &&
            (
                ...
                !$access->checkRbacOrPositionPermissionAccess('write', \ilObjUserFolder::ORG_OP_EDIT_USER_ACCOUNTS,
                    $this->usrf_ref_id) ||
                !in_array(
                    $this->object->getId(),
    ...
    public function updateObject()
    {
        ...
        if ($this->form_gui->checkInput()) {
            ...
            if ($this->isSettingChangeable('bs_allow_to_contact_me')) {
                $this->object->setPref('bs_allow_to_contact_me',
                    ($_POST['bs_allow_to_contact_me'] ?? false) ? 'y' : 'n');
            }
            if ($this->isSettingChangeable('chat_osc_accept_msg')) {
    ...
    public function getValues()
    {
        ...
        // system information
        $data["create_date"] = ilDatePresentation::formatDate(new ilDateTime($this->object->getCreateDate(),
            IL_CAL_DATETIME));
    ...
    public function assignSaveObject()
    {
        ...
        if (!$rbacsystem->checkAccess("edit_roleassignment", $this->usrf_ref_id)) {
            $this->ilias->raiseError($this->lng->txt("msg_no_perm_assign_role_to_user"),
                $this->ilias->error_obj->MESSAGE);
        }
        ...
        if ((empty($selected_roles) and count($assigned_roles_all) == count($assigned_roles))
            ...
            ilUtil::sendFailure($this->lng->txt("msg_min_one_role") . "<br/>" . $this->lng->txt("action_aborted"),
                true);
