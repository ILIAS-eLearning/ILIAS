 ...
 */
class ilPdoMySQLDatatype extends ilPdoDatatype {
...
class ilPdoDatatype {
		...
		foreach ($types as $key => $type) {
			if (!isset($this->valid_default_values[$type])) {
				...
				if (empty($db->options['datatype_map'][$type])) {
					return $db->raiseError(MDB2_ERROR_UNSUPPORTED, null, null, $type . ' for ' . $key
					                                                           . ' is not a supported column type', __FUNCTION__);
				}
			}
		}
	...
	function _getDeclarationOptions($field) {
		$charset = empty($field['charset']) ? '' : ' ' . $this->_getCharsetFieldDeclaration($field['charset']);
		...
		$collation = empty($field['collation']) ? '' : ' ' . $this->_getCollationFieldDeclaration($field['collation']);
