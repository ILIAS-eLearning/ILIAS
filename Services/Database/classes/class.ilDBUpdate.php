...
class ilDBUpdate {
	...
	public function getFileForStep($a_version) {
		...
		//
		switch (true) {
			case ((int)$a_version > 5296): // last number in previous file
	...
	public function loadXMLInfo() {
		...
		foreach ($modules as $module) {
			$mr = new ilModuleReader(
				ILIAS_ABSOLUTE_PATH . "/Modules/" . $module["subdir"] . "/module.xml", $module["subdir"], "Modules"
			);
			$mr->getModules();
			unset($mr);
		}
		...
		foreach ($services as $service) {
			$sr = new ilServiceReader(
				ILIAS_ABSOLUTE_PATH . "/Services/" . $service["subdir"] . "/service.xml", $service["subdir"], "Services"
			);
			$sr->getServices();
			unset($sr);
		}
	...
	public function setHotfixCurrentVersion($a_version) {
		...
		$this->hotfix_setting->set(
			"db_hotfixes_" . $this->hotfix_version[0] . "_" . $this->hotfix_version[1], $a_version
		);
