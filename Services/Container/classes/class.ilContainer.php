...
class ilContainer extends ilObject
{
	...
	*/
	...
	public static function parseCreatableSubObjects($a_parent_ref_id, array $a_disabled = null)
	{
		global $objDefinition, $lng, $ilAccess;
		
		if(!is_array($a_disabled))
		{
			$a_disabled = array();	
		}
		$a_disabled[] = "rolf";
		...
		{			
			...
			{								
				if (!in_array($type, $a_disabled))
				{
					// #9950
					if ($ilAccess->checkAccess("create_".$type, "", $a_parent_ref_id, $parent_type))
					{
