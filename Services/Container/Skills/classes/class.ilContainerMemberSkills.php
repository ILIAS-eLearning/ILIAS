...
class ilContainerMemberSkills
{
	 ...
	 * @param int $a_obj_id
	 */
	function __construct($a_obj_id, $a_user_id)
	{
		global $DIC;
	...
	function read()
	{
		...
		$set = $db->query("SELECT * FROM cont_member_skills ".
	 ...
	 */
	function saveLevelForSkills($a_level_data)
	{
		$db = $this->db;
	 ...
	 */
	function removeAllSkillLevels()
	{
		ilBasicSkill::removeAllUserSkillLevelStatusOfObject($this->getUserId(), $this->getObjId(), false,
			$this->getObjId());
