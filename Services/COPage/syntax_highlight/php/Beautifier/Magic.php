...
class Magic
{
    ...
    public function _is_ascii($file)
    {
        $arr = unpack("C*", $file);
        for ($i=1; $i<sizeof($arr); $i++) {
            if ($arr[$i]<32 && $arr[$i]!=13 && $arr[$i]!=10) {
                return false;
            }
        }
        return true;
    ...
    public function get_language($filename)
    {
        ...
        if (isset($langstr)) {
            ...
            foreach ($langarr as $lang) {
                ...
                } elseif (in_array($lang, $this->config->binfunctions)) {
                    array_push($outarray, $lang);
                }
            }
            $testarray = $outarray;
            ...
            if ($this->is_ascii) {
                $testarray = $this->config->langfunctions;
            } else {
                $testarray = $this->config->binfunctions;
            }
        }
