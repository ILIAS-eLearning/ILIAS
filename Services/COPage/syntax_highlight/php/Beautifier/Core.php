...
class Core
{
    public function __construct($file=undef, $outputmodule)
    {
        if (!isset($file)) {
            $file = new HFile();
        }
    ...
    public function set_stats($statobj)
    {
        $this->statobj = $statobj;
    }

    // Do the brunt work of highlighting the text.
    public function highlight_text($text, $contextstack=undef)
    {
        global $BEAUT_PATH;
        ...
        if (isset($this->highlightfile->zones)) {
            ...
            foreach ($this->highlightfile->zones as $zone) {
                ...
                if (!isset($this->endtags[$zone[0]])) {
                    $this->endtags[$zone[0]] = array();
                }
        ...
        if (isset($this->context->code_parts[0])) {
        ...
        for ($i=0; $i<$aln; $i++) {
            ...
            if ($this->highlightfile->notrim==0) {
                $line = trim($line);
            }
            ...
            for ($j=0; $j<$ln; $j++) {
                ...
                if (!$this->context->lineselect && !$this->context->inselection && !$this->context->inquote && !$this->context->incomment && !($this->highlightfile->perl && $j>0 && $line[$j-1]=="$")) {
                    ...
                    if ($currmax != 0) {
                        ...
                        if (isset($this->statobj) && $this->statobj->harvest_comments) {
                            $this->statobj->comment_cache .= " " . substr($line, $lncl);
                        }
                        $lineout = "";
                ...
                if ($this->context->incomment || $this->context->inbcomment) {
                    ...
                    if ($this->context->newline) {
                        if (isset($this->statobj) && $this->statobj->harvest_comments) {
                            $this->statobj->comment_cache .= " ";
                        }
                        $this->context->newline = 0;
                    }
                    if (isset($this->statobj) && $this->statobj->harvest_comments) {
                        $this->statobj->comment_cache .= $currchar;
                    }
                    continue;
                }
...
//print "14:$currchar";
                    ...
                    if (!$this->context->escaping && isset($this->context->inquote) && !$this->context->inquote) {
                        ...
                        if (isset($this->statobj) && $this->statobj->harvest_strings) {
                            $this->string_cache.=" ";
                        }
                        if ($this->context->prepro) {
                    ...
                    elseif ($this->context->inquote && !$this->context->escaping && $currchar == $this->context->currquotechar) {
                        //print "16";
                ...
                elseif (!$this->context->inselection && $delim && !$this->context->inquote && ($stri=$this->_starts_with(substr($line, $j), $this->highlightfile->indent))!="") {
                    //print "17";
                ...
                elseif (!$this->context->inselection && $delim && $this->context->ind>0 && !$this->context->inquote && ($stru=$this->_starts_with(substr($line, $j), $this->highlightfile->unindent))!="") {
                    //print "18";
                    $this->context->ind--;
                ...
                elseif (!$this->context->inwhitespace || $currchar != " " || $currchar != "\t") {
                    //print "19";
                ...
                } elseif ($this->context->inquote && $currchar == $this->highlightfile->escchar && !$this->context->escaping) {
                    //print "21";
        ...
        if ($this->context->incomment) {
            $out.= $this->context->linecomment_parts[1];
        } elseif ($this->context->inbcomment) {
            $out.= $this->context->blockcomment_parts[1];
        } elseif ($this->context->inselection) {
            $out.= $this->context->select_parts[1];
        }
        if (isset($this->context->code_parts[1])) {
    ...
    public function _munge($munge)
    {
        ...
        $strout 	= "";
        $lngth 		= strlen($munge);
        if ($this->context->inselection || $this->context->lineselect) {
            return $munge;
        }
        if (!$this->context->prepro) {
            for ($i=0; $i<=$lngth; $i++) {
                ...
                if ($delim || $i==($lngth)) {
                    if ($inword) {
                        ...
                        if (isset($this->context->validkeys[$checkword])) {
                            if ($this->highlightfile->nocase) {
                                $checkword = $this->context->validkeys[$checkword];
                            }
    ...
    // this returns 'foot'.
    public function _starts_with($text, $array)
    {
        $ml = 0;
        $curr = "";
