...
class MagicConfig
{
    ...
    public function detect_cpp()
    {
        return !(strpos($this->file, "//") === false);
    }
    
    public function detect_awk()
    {
    ...
    public function detect_perl()
    {
        if (preg_match("/^#!\s+\/bin\/perl/", $this->file)
            ...
            || preg_match("/^eval\s+\"exec\s+\/usr\/local\/bin\/perl/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_bash()
    {
        if (preg_match("/^#!\s+\/bin\/bash/", $this->file)
            || preg_match("/^#!\s+\/usr\/local\/bin\/bash/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_rc()
    {
        if (preg_match("/^#!\s+\/bin\/rc/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_env()
    {
        if (preg_match("/^#!\s*\/usr\/bin\/env/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_genericshell()
    {
    ...
    public function detect_xdiff()
    {
        if (preg_match("/^%XDZ/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_swf()
    {
        if (preg_match("/^FWS/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_psfont()
    {
        if (preg_match("/^(.*?)PS-AdobeFont-1\.0/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_lex()
    {
        if (substr($this->file, 53, 10)=="yyprevious"
            || substr($this->file, 21, 17)=="generated by flex") {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_lisp()
    {
        return !(strpos($this->file, ";;") === false);
    }
    
    public function detect_magic()
    {
    ...
    public function detect_xml()
    {
        if (preg_match("/\s*<\?xml/", $this->file)) {
            return true;
        } else {
            return false;
        }
    ...
    public function detect_bmp()
    {
        return (substr($this->file, 0, 2) == "BM");
    }

    public function detect_tif()
    {
