...
class ilAsqHintsTableGUI extends ilTable2GUI
{
	...
	public function __construct($parent_gui, $parent_cmd, QuestionDto $qustion_dto)
	{
		...
		$this->setTitle( sprintf($lng->txt('tst_question_hints_table_header'), $qustion_dto->getData()->getTitle()));
	...
	public function fillRow($row_data)
	{
		...
		if( $this->tableMode == self::TBL_MODE_ADMINISTRATION )
		{
			...
			$list->setId("advsl_hint_{$row_data->getOrderNumber()}_actions");
			...
			$hintIndex = $row_data->getOrderNumber() * self::INDEX_TO_POSITION_FACTOR;
		...
		$this->tpl->setVariable('HINT_TEXT', $row_data->getContent());
		$this->tpl->setVariable('HINT_POINTS', $row_data->getPointDeduction());
	}
}
