	...
	public function renderItemList($results)
	{
		...
		foreach($this->result->getTopics() as $oTopic)
		{
			...
			foreach($this->type_ordering as $act_type)
			{
				...
				if(count($results[$oTopic->getId()][$act_type]))
				{
					foreach($results[$oTopic->getId()][$act_type] as $key => $item)
					{
						$oPaymentObject =
						...
						if(is_object($md_section = $md_obj->getLifecycle()))
						{
							...
							$results[$oTopic->getId()][$act_type][$key]['author'] = $ent_str;
						}				
					}
					
					$results[$oTopic->getId()][$act_type] = $this->sortResult($results[$oTopic->getId()][$act_type]);				
			...
			if($items_counter > 0)
			{
				...
				$oContainerTpl->setVariable('TOPIC_TITLE', $oTopic->getTitle());
