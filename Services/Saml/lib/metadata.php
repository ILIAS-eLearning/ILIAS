...
// ilias-patch: begin
$slol = $iliasHttpPath . '/saml2-logout.php/'.$sourceId . '?cliend_id=' . CLIENT_ID;
// ilias-patch: end
...
foreach ($assertionsconsumerservices as $services) {
	...
	switch ($services) {
		...
		case 'urn:oasis:names:tc:SAML:1.0:profiles:artifact-01':
			...
			// ilias-patch: begin
			$acsArray['Location'] = $iliasHttpPath . "/saml1-acs.php/{$sourceId}/artifact?cliend_id=" . CLIENT_ID;
			// ilias-patch: end
			break;
...
if (array_key_exists('output', $_REQUEST) && $_REQUEST['output'] == 'xhtml') {
	...
	// ilias-patch: begin
	$t->data['metaurl'] = $iliasHttpPath . '/metadata.php?cliend_id=' . CLIENT_ID;
	// ilias-patch: end
	$t->show();
} else {
