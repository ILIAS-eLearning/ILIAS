...
class sspmod_saml_Auth_Source_SP extends SimpleSAML_Auth_Source {
	...
	public function getIdPMetadata($entityId) {
		...
		if ($this->idp !== NULL && $this->idp !== $entityId) {
			// saml-patch: begin
			SimpleSAML_Logger::debug('Sending SAML 2 Checkeding entityId ' . var_export($entityId, true) . ' against ' . var_export($this->idp, true));
			$entityUrl = @parse_url($this->idp);
			if(is_array($entityUrl)) {
				if ($entityUrl['scheme'] == 'http') {
					$entityId = str_replace('https://', 'http://', $entityId);
				} else {
					$entityId = str_replace('http://', 'https://', $entityId);
				}
				SimpleSAML_Logger::debug('Sending SAML 2 Checkeding entityId ' . var_export($entityId, true) . ' against ' . var_export($this->idp, true));
			}
