...
if (isset($_FILES['img_file']['size']) && $_FILES['img_file']['size']>0)
{
	...
	$media_object = saveTempFileAsMediaObject($_GET["session_id"]."::".$_GET["client_id"], $safefilename, $_FILES["img_file"]["tmp_name"]);
...
if ($_GET["update"] == 1)
{
	$tpl->setVariable("INSERT_COMMAND", "{\$lang_update}");
}
else
{
	$tpl->setVariable("INSERT_COMMAND", "{\$lang_insert}");
}
