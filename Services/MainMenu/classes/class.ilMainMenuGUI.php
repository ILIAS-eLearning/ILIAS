...
class ilMainMenuGUI
{
	...
	*/
	...
	{
		global $ilias, $rbacsystem, $ilUser, $ilLog;
		//$ilLog->write("YYYYY: __construct");
	...
	{
		global $DIC;
		/* don't change anything in normal FULL view mode since view modes are fully supported. */
	...
	*/
	...
	{
		global $rbacsystem, $lng, $ilias, $tree, $ilUser, $ilSetting, $ilPluginAdmin, $DIC, $ilLog;
		...
		if (isset($_SESSION['il_view_mode']) && $_SESSION['il_view_mode'] !== 'ilFullViewGUI') {
			...
			$main_menu_list_entries = $view->hookMainMenuListEntries();
			switch ($main_menu_list_entries) { // only KEEP supported since extra Blockfile exists for view Hook
				case $view::KEEP :
					$mmle_html = "";
			...
			// search
			$search = $view->hookSearch();
			switch ($search) { // only KEEP supported since extra Blockfile exists for view Hook
				case $view::KEEP :
			...
			// statusbox
			$statusbox = $view->hookStatusbox();
			switch ($statusbox) { // only KEEP supported since extra Blockfile exists for view Hook
				case $view::KEEP :
			...
			// user_logged_in
			$user_logged_in = $view->hookUserLoggedIn();
			switch ($user_logged_in) { // only KEEP supported since extra Blockfile exists for view Hook
				case $view::KEEP : 
					$this->tpl->addBlockFile("USERLOGGEDIN","userisloggedin","tpl.user_logged_in.html","Services/MainMenu");
					...
					break;
				case $view::REPLACE : // ToDo: Concept
					$view->replace($this->tpl,'user_logged_in');
					break;
			}
			
			// top_header_bar
			$top_bar_header = $view->hookTopBarHeader();
			
			switch ($top_bar_header) { 
				case $view::KEEP :
					if(!$this->topbar_back_url)
					{
						include_once("./Modules/SystemFolder/classes/class.ilObjSystemFolder.php");
						$header_top_title = ilObjSystemFolder::_getHeaderTitle();
						if (trim($header_top_title) != "")
						{
							$this->tpl->addBlockFile("HEADER_TOP_TITLE","header_top_title","tpl.header_top_title.html","Services/MainMenu");
					...
					{
						$this->tpl->addBlockFile("HEADER_BACK_BL","header_back_bl","tpl.header_back_bl.html","Services/MainMenu");
		...
		else { // OLD CODE! don't change anything in normal FULL view mode since view modes are fully supported.
			// changed only the new blockFiles for a modular design
			if($this->logo_only)
			{		
