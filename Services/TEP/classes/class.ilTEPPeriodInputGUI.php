 ...
 * @ingroup ServicesTEP
 */
class ilTEPPeriodInputGUI extends ilFormPropertyGUI
{
	...
	protected function render()
	{
		...
		$tpl = new ilTemplate("tpl.period_input.html", true, true, "Services/TEP");
		...
		if($this->mode == self::MODE_OVERNIGHT)
		{			
			$tpl->setVariable("OV_TXT_HEAD_FROM", $lng->txt("tep_period_input_from"));
			$tpl->setVariable("OV_TXT_HEAD_TO", $lng->txt("tep_period_input_to"));		
			...
			{			
				...
				{										
					...
					$tpl->setVariable("OV_TXT_TO",  $lng->txt("tep_period_input_to_last"));				
				}
				// night in period
				...
				{				
					$tpl->setVariable("OV_TXT_FROM",  $lng->txt("tep_period_input_from_first"));	
					$tpl->setVariable("OV_TXT_TO", ilDatePresentation::formatDate($tomorrow));					
				}
