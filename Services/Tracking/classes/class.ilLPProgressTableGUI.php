...
class ilLPProgressTableGUI extends ilTable2GUI
{
	...
	* Constructor
	*/
	function __construct($a_parent_obj, $a_parent_cmd, $a_user = "")
	{
		...
		$this->addColumn($this->lng->txt("trac_percentage"), "percentage", "10%");
			...
			foreach($path_full as $data)
			{
				...
				else
				{
					...
					$path .= ('<a target="_top" href="'.
							  $ilCtrl->getLinkTargetByClass($ilCtrl->getCmdClass(), 'details').'">'.
							  $data['title'].'</a>');
				}
			}
