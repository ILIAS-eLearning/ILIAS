...
class ilLMStatistics {
	...
	function calcStats($mode) {
		...
		if (count($user_IDs) > 0 || ($user_selection == "all")) {
			...
			$SeitenVerweildauerVarianz = varianzSV($SessionsVerweildauer, $SeitenVerweildauerDurchschnitt);
			...
			if ($Seitenanz > 0) {
				$SeitenZugriffeVarianz = varianzSV($SeitenZugriffMax, $GesamtSeitenZugriffe / $Seitenanz);
			}
			...
			for ($i = 1; $i <= $Seitenanz; $i ++) {
				if ($SeitenMessbareZugriffe[$i] > 0) {
					$EinzelSeitenVarianz[$i] = varianzSV($SeitenVerweildauerListe[$i], $SeiteGesamtVerweilZeit[$i] / $SeitenMessbareZugriffe[$i]);
			...
			*	Soll zB eine neue Option hinzugefügt werden kann man das mit den Einträgen...
			...
			$SeitenStatsWert[$slider] = s_to_h($SessionGesamtDauerAll)." (".round($SessionGesamtDauerAll)."s)";
			$slider ++;
			if ($Seitenanz > 0) {
			...
			$SessionStatsWert[$slider2] = s_to_h($SessionGesamtDauerAll)." (".$SessionGesamtDauerAll.$lng->txt("stats_sec").")";
			$slider2 ++;
			$SessionStatsName[$slider2] = $lng->txt("stats_session_average_length"); //"durchschn. Sessionlaenge";
			$SessionStatsWert[$slider2] = s_to_m($SessionDurchschnittsDauer);
			$slider2 ++;
			...
			for ($i = 0; $i < $RankAnzahl; $i ++) {
				...
				$KapitelStatsWert[$i +1 + $slider3] = s_to_h($KapitelGesamtVerweilMin[$i]);
			}
			$slider3 += $RankAnzahl + $hop;
			...
			for ($i = 1; $i <= $Kapitelanz; $i ++) {
				if ($seitenTiefe[$KapitelID[$i]] == 2) {
					if ($KapitelSeitenSumme[$i] > 0) {
						...
						$KapitelStatsWertA1[$u +1 + $slider3] = proz(floor($KapitelAbgedeckteSeiten1[$i] * 100 / $KapitelSeitenSumme[$i]))."%(".proz($KapitelAbgedeckteSeiten1[$i]).")";
						$KapitelStatsWertA2[$u +1 + $slider3] = proz(floor($KapitelAbgedeckteSeiten2[$i] * 100 / $KapitelSeitenSumme[$i]))."%(".proz($KapitelAbgedeckteSeiten2[$i]).")";
						$KapitelStatsWertA3[$u +1 + $slider3] = proz(floor($KapitelAbgedeckteSeiten3[$i] * 100 / $KapitelSeitenSumme[$i]))."%(".proz($KapitelAbgedeckteSeiten3[$i]).")";
						$KapitelStatsWertA4[$u +1 + $slider3] = proz(floor($KapitelAbgedeckteSeiten4[$i] * 100 / $KapitelSeitenSumme[$i]))."%(".proz($KapitelAbgedeckteSeiten4[$i]).")";
						$KapitelStatsWertA5[$u +1 + $slider3] = proz(floor($KapitelAbgedeckteSeiten5[$i] * 100 / $KapitelSeitenSumme[$i]))."%(".proz($KapitelAbgedeckteSeiten5[$i]).")";
					}
				}
			}
			$slider3 += $u + $hop +2;
