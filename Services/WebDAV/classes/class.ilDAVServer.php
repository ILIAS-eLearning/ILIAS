...
class ilDAVServer extends HTTP_WebDAV_Server
{
    ...
    */
    private function __construct()
    {
        ...
        } elseif (strpos($userAgent, 'darwin') !== false
    ...
    */
    public function GET(&$options)
    {
        ...
        if ($objDAV->isCollection()) {
            ...
            } elseif (isset($_GET['mount-instructions'])) {
    ...
    */
    private function getDir(&$objDAV, &$options)
    {
        ...
        echo "<p><font size=\"-1\">" .
                sprintf(
                    $lng->txt('webfolder_mount_dir_with'),
    ...
    */
     ...
     */
    public function getMountURI($refId, $nodeId = 0, $ressourceName = null, $parentRefId = null, $genericURI = false)
    {
        if ($genericURI) {
            $baseUri = ($this->isWebDAVoverHTTPS() ? "https:" : "http:");
            $query = null;
        } elseif ($this->clientOS == 'windows') {
            $baseUri = ($this->isWebDAVoverHTTPS() ? "https:" : "http:");
            $query = 'mount-instructions';
        } elseif ($this->clientBrowser == 'konqueror') {
            $baseUri = ($this->isWebDAVoverHTTPS() ? "webdavs:" : "webdav:");
            $query = null;
        } elseif ($this->clientBrowser == 'nautilus') {
    ...
    */
    public static function _getWebfolderInstructionsFor(
        $webfolderTitle,
        $webfolderURI,
        $webfolderURI_IE,
        $webfolderURI_Konqueror,
