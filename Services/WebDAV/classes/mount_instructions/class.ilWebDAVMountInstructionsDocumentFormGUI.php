...
class ilWebDAVMountInstructionsDocumentFormGUI extends ilPropertyFormGUI
{
    ...
    protected function initForm() : void
    {
        ...
        // TODO: Fix language selection from lng (this is from meta, not form)
        $options = [];
        ...
        $language_selection->setValue((string) ($config['lng'] ?? ''));

        $this->addItem($language_selection);

        $document = new ilFileStandardDropzoneInputGUI($document_label, 'document');
        $document->setInfo($document_by_line);
        if($document_already_exists)
        {
            $document->setRequired(true);
        }
        $document->setDisabled(!$this->is_editable);
    ...
    protected function createFilledObject(ilWebDAVMountInstructionsDocument $document) : ilWebDAVMountInstructionsDocument
    {
        ...
        $id = $this->mount_instructions_repository->getNextMountInstructionsDocumentId();
    ...
    protected function getRawMountInstructionsFromFileUpload(UploadResult $upload_result) : string
    {
        ...
        $this->file_upload->moveOneFileTo(
            $upload_result, '/mount_instructions', Location::TEMPORARY, '', true
        );
    ...
    protected function getFileUploadResult() : UploadResult
    {
        ...
        } else if($this->file_upload->hasBeenProcessed())
        {
            throw new InvalidArgumentException("");
        }
