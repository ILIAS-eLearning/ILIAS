 ...
 */
class ilAdvancedMDFieldDefinitionMultiSelect extends ilAdvancedMDFieldDefinition
{		
	...
	{		
		...
		{			
			...
			{					
				...
				$in_use = ilADTActiveRecordByType::readByPrimary("adv_md_values", $primary, "MultiEnum");				
				if($in_use)
				{
			...
			foreach($this->confirmed_objects as $old_option => $obj_ids)
			{
				foreach($obj_ids as $obj_id => $new_option)
				{
					...
					else
					{
						...
						ilADTActiveRecordByType::writeByPrimary("adv_md_values", $primary, "MultiEnum", $new_option);
					}
				}
			}			
		}		
