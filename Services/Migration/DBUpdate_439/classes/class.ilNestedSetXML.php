...
class ilNestedSetXML
{
    ...
    *   @access	public
    */
    public function import($xmldata, $obj_id, $obj_type)
    {
        // {{{
        /**
    ...
    *   @access    public
    */
    public function export($obj_id, $type)
    {
        // {{{
        global $ilDB;
    ...
    *   @access	public
    */
    public function init($obj_id, $obj_type)
    {
        global $ilDB;
    ...
    */
    public function setTagValue($value)
    {
        ...
        if (is_array($row = $result->fetchRow(ilDBConstants::FETCHMODE_ASSOC))) {
    ...
    *   @access    public
    */
    public function getXpathNodes(&$doc, $qry)
    {
        if (is_object($doc)) {
    ...
    *   @access    public
    */
    public function addXMLNode($xPath, $xml, $index = 0)
    {
    ...
    *   @access    public
    */
    public function getFirstDomContent($xPath)
    {
    ...
    *   @access    public
    */
    public function deleteDomNode($xPath, $name, $index = 0)
    {
        if ($index == "") {
            $index = 0;
        }
    ...
    *   @access    public
    */
    public function addDomNode($xPath, $name, $value = "", $attributes = "", $index = 0)
    {
        $nodes = $this->getXpathNodes($this->dom, $xPath);
        if (count($nodes) > 0) {
    ...
    *   @access    public
    */
    public function updateDomNode($xPath, $meta, $no = 0)
    {
        $this->clean($meta);
        $update = false;
        ...
        if (count($nodes) > 0) {
            ...
            if ($nodes[0]->node_name() == "BibItem") {
                ...
                } elseif ($meta["URL_ISBN_ISSN"]["Type"] == "ISBN") {
                    $xml .= '<ISBN>' . ilUtil::stripSlashes($meta["URL_ISBN_ISSN"]["Value"]) . '</ISBN>';
                } elseif ($meta["URL_ISBN_ISSN"]["Type"] == "ISSN") {
            ...
            /* General */
            elseif ($nodes[0]->node_name() == "General") {
            ...
            /* Lifecycle */
            elseif ($nodes[0]->node_name() == "Lifecycle") {
            ...
            /* Meta-Metadata */
            elseif ($nodes[0]->node_name() == "Meta-Metadata") {
            ...
            /* Technical */
            elseif ($nodes[0]->node_name() == "Technical") {
                $xml = '<Technical>';
                ...
                } elseif (is_array($meta["OrComposite"])) {
            ...
            /* Educational */
            elseif ($nodes[0]->node_name() == "Educational") {
            ...
            /* Rights */
            elseif ($nodes[0]->node_name() == "Rights") {
            ...
            /* Relation */
            elseif ($nodes[0]->node_name() == "Relation") {
...
#				for ($j = 0; $j < count($meta["Relation"]); $j++)
            ...
            /* Annotation */
            elseif ($nodes[0]->node_name() == "Annotation") {
...
#				for ($i = 0; $i < count($meta["Annotation"]); $i++)
            ...
            /* Classification */
            elseif ($nodes[0]->node_name() == "Classification") {
...
#				for ($j = 0; $j < count($meta["Classification"]); $j++)
    ...
    *   @access    public
    */
    public function getDomContent($xPath, $name = "", $index = 0)
    {
        if ($index == "") {
            $index = 0;
        }
    ...
    *   @access    public
    */
    public function replaceDomContent($xPath, $name = "", $index = 0, $newNode)
    {
        #		echo "Index: " . $index . " | Path: " . $xPath . " | Name: " . $name . "<br>\n";
        $nodes = $this->getXpathNodes($this->dom, $xPath);
        if (count($nodes) > 0) {
    ...
    *   @access    public
    */
    public function updateDomContent($xPath, $name = "", $index = 0, $newNode)
    {
        //		echo "Index: " . $index . " | Path: " . $xPath . " | Name: " . $name . "<br>\n";
        $nodes = $this->getXpathNodes($this->dom, $xPath);
        if (count($nodes) > 0) {
    ...
    *   @access    public
    */
    public function getFirstDomNode($xPath)
    {
        $node = $this->getXpathNodes($this->dom, $xPath);
        return($node[0]);
    }
    ...
    *   @access    private
    */
    public function deleteAllDbData()
    {
        global $ilBench, $ilDB;
    ...
    *	@return boolean
    */
    public function _deleteAllChildMetaData($a_ids)
    {
        global $ilBench, $ilDB;
    ...
    *	@return boolean
    */
    public function _getAllChildIds($a_obj_id)
    {
        global $ilDB;
