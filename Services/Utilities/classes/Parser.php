...
class Parser
{
    ...
    public $mDefaultSort;
    public $mTemplates;
    // cache of already loaded templates, avoids
                        // multiple SQL queries for the same string
        public $mTemplatePath;	// stores an unsorted hash of all the templates already loaded
    ...
    public $ot;
    // Shortcut alias, see setOutputType()
    public $mRevisionId;
     ...
     */
    public function preprocess($text, $title, $options)
    {
        wfProfileIn(__METHOD__);
        $this->clearState();
     ...
     * @private
     */
    public function strip($text, $state, $stripcomments = false, $dontstrip = array())
    {
        global $wgContLang;
        wfProfileIn(__METHOD__);
     ...
     */
    public function unstrip($text, $state)
    {
     ...
     * @deprecated use $this->mStripState->unstrip()
     */
    public function unstripNoWiki($text, $state)
    {
     ...
     */
    public function unstripForHTML($text)
    {
     ...
     * @private
     */
    public function insertStripItem($text, &$state)
    {
...
'<head><title>test</title></head><body>' . $text . '</body></html>';
     ...
     * @static
     */
    public function externalTidy($text)
    {
     ...
     * @static
     */
    public function internalTidy($text)
    {
        global $wgTidyConf;
     ...
     * @private
     */
    public function internalParse($text)
    {
        $args = array();
        $isMain = true;
     ...
     * @private
     */
    public function replaceFreeExternalLinks($text)
    {
        global $wgContLang;
    ...
    public static function replaceUnusualEscapes($url)
    {
        return preg_replace_callback(
            '/%[0-9A-Fa-f]{2}/',
            array( 'Parser', 'replaceUnusualEscapesCallback' ),
            $url
        );
    }
     ...
     *
     */
    public function makeLinkHolder(&$nt, $text = '', $query = '', $trail = '', $prefix = '')
    {
        wfProfileIn(__METHOD__);
        if (!is_object($nt)) {
            # Fail gracefully
     ...
     * @private
     */
    /* private */ public function closeParagraph()
    {
        $result = '';
    ...
    #
    /* private */ public function getCommon($st1, $st2)
    {
        $fl = strlen($st1);
        $shorter = strlen($st2);
        if ($fl < $shorter) {
    ...
    #
    /* private */ public function openList($char)
    {
    ...
    /* private */ public function nextItem($char)
    {
        if ('*' == $char || '#' == $char) {
            return '</li><li>';
        } elseif (':' == $char || ';' == $char) {
    ...
    /* private */ public function closeList($char)
    {
        if ('*' == $char) {
            $text = '</li></ul>';
        } elseif ('#' == $char) {
     ...
     * @private
     */
    public function getVariableValue($index)
    {
     ...
     * @private
     */
    public function replace_callback($text, $callbacks)
    {
        wfProfileIn(__METHOD__);
            ...
            if ($found == 'open') {
                ...
                if (null === $openingBraceStack[$lastOpeningBrace]['parts']) {
                    $openingBraceStack[$lastOpeningBrace]['title'] =
                        substr(
                            $text,
                            $openingBraceStack[$lastOpeningBrace]['partStart'],
     ...
     * @private
     */
    public function replaceVariables($text, $args = array(), $argsOnly = false)
    {
     ...
     * @private
     */
    public function variableSubstitution($matches)
    {
        global $wgContLang;
     ...
     * @private
     */
    public function braceSubstitution($piece)
    {
     ...
     * @private
     */
    public function argSubstitution($matches)
    {
        ...
        if (array_key_exists($arg, $inputArgs)) {
            $text = $inputArgs[$arg];
        } elseif (($this->mOutputType == OT_HTML || $this->mOutputType == OT_PREPROCESS) &&
     ...
     */
    public function stripNoGallery(&$text)
    {
     ...
     * @private
     */
    public function pstPass2($text, &$stripState, $user)
    {
     ...
     * @private
     */
    public function getUserSig(&$user)
    {
     ...
     */
    public function cleanSigInSig($text)
    {
        ...
        if (!empty($this->mLinkHolders['namespaces'])) {
            ...
            foreach ($this->mLinkHolders['namespaces'] as $key => $ns) {
                ...
                if (empty($colours[$pdbk])) {
                    ...
                    $replacePairs[$searchkey] = $sk->makeBrokenLinkObj(
                        $title,
                        $this->mLinkHolders['texts'][$key],
                        $this->mLinkHolders['queries'][$key]
                    );
                } elseif ($colours[$pdbk] == 1) {
                    $replacePairs[$searchkey] = $sk->makeKnownLinkObj(
                        $title,
                        $this->mLinkHolders['texts'][$key],
                        $this->mLinkHolders['queries'][$key]
                    );
                } elseif ($colours[$pdbk] == 2) {
                    $replacePairs[$searchkey] = $sk->makeStubLinkObj(
                        $title,
                        $this->mLinkHolders['texts'][$key],
                        $this->mLinkHolders['queries'][$key]
                    );
     ...
     */
    public function renderPreTag($text, $attribs)
    {
     ...
     */
    public function makeImage($nt, $options)
    {
     ...
     * @private
     */
    public function attributeStripCallback(&$text, $args)
    {
        $text = $this->replaceVariables($text, $args);
        $text = $this->mStripState->unstripBoth($text);
        return $text;
