...
*/
	...
	*/
	...
	function processZipFile ($a_directory, $a_file, $structure, $ref_id = null, $containerType = null) {
		...
		ilUtil::recursive_dirscan($a_directory, $filearray);
		...
		if ($ref_id != null)
		{
			ilUtil::createObjects ($a_directory, $structure, $ref_id, $containerType);
		}
		return 0;
	...
	function recursive_dirscan($dir, &$arr)
	{
	  	...
	  	while ($file = readdir ($dirlist))
		{
			if ($file != '.' && $file != '..')
			{
				...
				if (is_dir($newpath))
				{
					ilUtil::recursive_dirscan($newpath, $arr);
				}
				else
				{
					$arr["path"][] = $dir . "/";
	  	...
	  	while ($file = readdir ($dirlist))
		{
			if ($file != '.' && $file != '..')
			{
				...
				if (is_dir($newpath))
				{
					if ($structure) 
					{
						$new_ref_id = ilUtil::createContainer($file, $ref_id, $containerType);
						ilUtil::createObjects($newpath, $structure, $new_ref_id, $containerType);
					}
					else 
					{
						ilUtil::createObjects($newpath, $structure, $ref_id, $containerType);
					}
				}
				else
				{
					ilUtil::createFile (end($level),$dir,$ref_id);
	      			}
			}
		}
		closedir($dirlist);		
