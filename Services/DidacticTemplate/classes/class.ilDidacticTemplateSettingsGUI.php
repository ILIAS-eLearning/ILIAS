...
class ilDidacticTemplateSettingsGUI
{
	...
	protected function initEditTemplate(ilDidacticTemplateSetting $set)
	{
		...
		$title->setValue($set->isAutoGenerated() ? $set->getPresentationTitle() : $set->getTitle());
		$title->setDisabled($set->isAutoGenerated());

		if(!$set->isAutoGenerated())
		{
			$title->setInfo($this->getTranslationInfo($set));
		}
		...
		//use presentation title if autogenerated is set
		$desc->setValue($set->isAutoGenerated() ? $set->getPresentationDescription() : $set->getDescription());
		$desc->setRows(3);
	...
	protected function getTranslationInfo(ilDidacticTemplateSetting $a_set)
	{
		global $ilCtrl;
		$trans = $a_set->getTranslations();
		$def = $trans[0]; // default
		...
		if(sizeof($trans) > 1)
		{
			...
			return ($this->lng->txt("language").": ".$languages[$def["lang_code"]].
