...
class SurveyImportParserPre38 extends ilSaxParser
{
    ...
    * @access	public
    */
    public function __construct($a_spl_id, $a_xml_file = '', $spl_exists = false)
    {
        die("Class surveyImportParserPre38 is deprecated.");
        ...
        switch ($a_name) {
            ...
            case "qticomment":
                if (strcmp($this->getParent($a_xml_parser), "item") == 0) {
                    ...
                    } elseif (preg_match("/Author\=(.*)/", $this->characterbuffer, $matches)) {
                        if (is_object($this->activequestion)) {
                            $this->activequestion->setAuthor($matches[1]);
                        }
                    } elseif (preg_match("/ILIAS Version\=(.*)/", $this->characterbuffer, $matches)) {
                    } else {
