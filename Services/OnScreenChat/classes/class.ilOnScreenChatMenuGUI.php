...
class ilOnScreenChatMenuGUI
{
    ...
    public function getMainMenuHTML()
    {
        ...
        $config = array(
            'conversationTemplate' => (new ilTemplate('tpl.chat-menu-item.html', false, false,
                'Services/OnScreenChat'))->get(),
            'roomTemplate' => (new ilTemplate('tpl.chat-menu-item-room.html', false, false,
                'Services/OnScreenChat'))->get(),
            'infoTemplate' => (new ilTemplate('tpl.chat-menu-item-info.html', false, false,
                'Services/OnScreenChat'))->get(),
        ...
        $glyph = $glyph->withCounter($f->counter()->status(0))->withCounter($f->counter()->novelty(0))->withOnLoadCode(function (
            $id
        ) use (&$glyph_id) {
            $glyph_id = $id;
            return '';
