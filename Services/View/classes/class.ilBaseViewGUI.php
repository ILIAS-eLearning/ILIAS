...
abstract class ilBaseViewGUI
{
	...
	*/
	...
	/**
	 * ToDo: Concept: Service or View itself should decide if the view is enabled from admin settings?
	 * if not enabled the view can not be activated, but is instantiated in DIC as ilViewNAME 
	 */ 
	public abstract function isEnabled();
	
	/*
	 * checkActivation in view constructor
	 * the activation logic must be implemented in the view
	 * @return bool
	 */ 
	public abstract function checkActivation();
	...
	*/
	...
	protected function getCmdLink($cmd) {
		...
		return $this->link_dir.$targetScript.$this->ctrl->getLinkTargetByClass(array('ilviewroutergui',strtolower(get_class($this))),$cmd)."&baseClass=ilviewroutergui";
	}
