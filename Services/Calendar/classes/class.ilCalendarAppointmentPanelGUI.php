...
class ilCalendarAppointmentPanelGUI
{
    ...
    public function getHTML($a_app)
    {
            ...
            case ilCalendarCategory::TYPE_OBJ:
                ...
                if ($this->settings->isCGRegistrationEnabled() and $type == 'crs' or $type == 'grp') {
                    if (!$a_app['event']->isAutoGenerated()) {
                        ...
                        if ($reg->isRegistered($this->user->getId(), new ilDateTime($a_app['dstart'], IL_CAL_UNIX),
                            new ilDateTime($a_app['dend'], IL_CAL_UNIX))) {
                            ...
                            $this->tpl->setVariable('PANEL_CANCELBOOK_HREF',
                            ...
                            $this->tpl->setVariable('PANEL_BOOK_HREF',
                                $this->ctrl->getLinkTargetByClass('ilcalendarappointmentgui', 'confirmRegister'));
                            $this->tpl->parseCurrentBlock();
                        }

                        $registrations = array();
                        foreach ($reg->getRegisteredUsers(new ilDateTime($a_app['dstart'], IL_CAL_UNIX),
                            new ilDateTime($a_app['dend'], IL_CAL_UNIX)) as $usr_data) {
                            $usr_id = $usr_data;
                            $this->ctrl->setParameterByClass('ilconsultationhoursgui', 'user', $usr_id);
                            $registrations[] = '<a href="' . $this->ctrl->getLinkTargetByClass('ilconsultationhoursgui',
                                    'showprofile') . '">' . ilObjUser::_lookupFullname($usr_id);
                            $this->ctrl->setParameterByClass('ilconsultationhoursgui', 'user', '');
                        }
                        if (count($registrations)) {
                            $this->tpl->setCurrentBlock('panel_current_booking');
                            $this->tpl->setVariable('PANEL_TXT_CURRENT_BOOKING',
                ...
                if (!$is_owner) {
                    ...
                    foreach ($entry->getCurrentBookings($a_app['event']->getEntryId()) as $user_id) {
                        if ($link_users) {
                            ...
                            $bookings[] = '<a href="' . $this->ctrl->getLinkTargetByClass('ilconsultationhoursgui',
                                    'showprofile') . '">' .
        ...
        if ($cat_info['editable'] and !$a_app['event']->isAutoGenerated()) {
            ...
            $this->tpl->setVariable('PANEL_EDIT_HREF',
            ...
            $this->ctrl->setParameterByClass('ilcalendarappointmentgui', 'app_id', $a_app['event']->getEntryId());
            $this->ctrl->setParameterByClass('ilcalendarappointmentgui', 'dt', $a_app['dstart']);
            $this->tpl->setVariable('PANEL_DELETE_HREF',
