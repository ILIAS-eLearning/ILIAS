...
class ilUserCertificateTableProvider
{
	...
	public function fetchDataSet($userId, $params, $filter)
	{
		...
		$sql = 'SELECT id, acquired_timestamp, obj_id FROM user_certificates WHERE user_id = ' . $this->database->quote($userId, 'integer') . ' AND currently_active = 1';
		...
		if(isset($params['limit'])) {
			$cnt_sql     = 'SELECT COUNT(*) cnt FROM user_certificates WHERE user_id = ' . $this->database->quote($userId, 'integer') . ' AND currently_active = 1';
		...
		$this->logger->info(sprintf('END - All active certificates for user: "%s" total: "%s"', $userId, count($data['cnt'])));
