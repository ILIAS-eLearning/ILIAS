...
class ilLicense
{
    ...
    /**
     * @param int $a_licenses
     */
    public function setLicenses($a_licenses = 0)
    {
     ...
     * @return int
     */
    public function getRemainingLicenses()
    {
     ...
     */
    public function getPotentialAccesses()
    {
        global $ilDB;
        ...
        $role_ids = array();
        $query = 'SELECT DISTINCT pa.rol_id' . ' FROM rbac_pa pa' . ' INNER JOIN object_reference ob ON ob.ref_id = pa.ref_id' . ' WHERE '
                 . $ilDB->like('pa.ops_id', 'text', '%%i:' . $ops_ids[0] . ';%%') . ' AND ob.obj_id = ' . $ilDB->quote($this->getObjId(), 'integer');
        ...
        $query = 'SELECT COUNT(DISTINCT(usr_id)) accesses ' . ' FROM rbac_ua' . ' WHERE ' . $ilDB->in('rol_id', $role_ids, false, 'integer')
                 . ' AND usr_id NOT IN' . ' (SELECT usr_id FROM read_event' . '  WHERE obj_id = ' . $ilDB->quote($this->getObjId(), 'integer') . ')';
    ...
    public function read()
    {
        ...
        $result = $ilDB->queryF($query, array( 'integer' ), array( $this->getObjId() ));
    ...
    public function update()
    {
        ...
        if ($row = $ilDB->fetchObject($result)) {
            $ilDB->update('license_data', array(
     ...
     *
     * @deprecated use ilLicenseAccess::_checkAccess instead
     *
     * @param    int $a_usr_id
     ...
     * @param int $a_ref_id
     */
    public static function _noteAccess($a_obj_id, $a_type, $a_ref_id)
    {
        global $ilDB, $ilUser;
     ...
     */
    public static function _getLicensedObjects()
    {
        global $ilDB;
        $objects = array();

        $query = 'SELECT od.obj_id, od.type, od.title, od.description, re.ref_id ' . 'FROM license_data ld '
                 . 'INNER JOIN object_data od ON od.obj_id = ld.obj_id ' . 'INNER JOIN object_reference re ON re.obj_id = od.obj_id '
                 . 'WHERE ld.licenses > 0 ' . 'ORDER BY od.title, od.obj_id';
     ...
     *
     * @param    int $a_ref_id of the repository node to check
     * @return   array        array of object data arrays (obj_id, title, desc)
     */
    public static function _getLicensedChildObjects($a_ref_id)
    {
        global $tree;
        $objects = array();
        ...
        foreach ($childs as $data) {
            if (in_array($data['type'], array( 'sahs', 'htlm' )) and self::_isLicensed($data['obj_id'])) {
                $objects[] = $data;
            }
        }
     ...
     */
    public static function _isLicensed($a_obj_id)
    {
        global $ilDB;

        $query = "SELECT licenses FROM license_data " . "WHERE obj_id = %s " . "AND licenses > 0";
