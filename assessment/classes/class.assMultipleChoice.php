  ...
  function save_to_db()
  {
    ...
    if ($result == DB_OK) {
      ...
      foreach ($this->answers as $key => $value) {
        ...
        $query = sprintf("INSERT INTO qpl_answers (answer_id, question_fi, answertext, points, `order`, correctness, TIMESTAMP) VALUES (NULL, %s, %s, %s, %s, %s, NULL)",
          $db->quote($this->id),
    ...
    if (strcmp(get_class($result), db_result) == 0) {
      ...
      if (strcmp(get_class($result), db_result) == 0) {
        while ($data = $result->fetchRow(DB_FETCHMODE_OBJECT)) {
          array_push($this->answers, new ASS_AnswerTrueFalse($data->answertext, $data->points, $data->order, $data->correctness));
        }
      }
    }
  }
