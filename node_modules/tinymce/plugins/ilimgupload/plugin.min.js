...
(function() {
    ...
    var hasImagesFileTypes = function(editor) {
        return !!getImageFileTypes(editor);
    };
    var getImageFileTypes = function(editor) {
        return editor.getParam('images_file_types', '', 'string')
    };
    var hasAllowedFileExtensionText = function(editor) {
        return !!getAllowedFileExtensionsText(editor);
    };
    var getAllowedFileExtensionsText = function(editor) {
        return editor.getParam('ilimgupload_file_extensions', '', 'string');

    };
    var hasMaxUploadSizeText = function(editor) {
        return !!getMaxUploadSizeText(editor);

    };
    var getMaxUploadSizeText = function(editor) {
        return editor.getParam('ilimgupload_maxsize', '', 'string');
    }
    ...
    var read = function(normalizeCss, image) {
        if (getAttrib(image, 'src').length > 0) {
            image_update = 1;
        }
        return {
    ...
    var write = function(normalizeCss, newData, image) {
        ...
        updateProp(image, oldData, newData, 'alignment', normalized(setAlignment, normalizeCss));
    };
    ...
    function Uploader(settings) {
        var defaultHandler = function(blobInfo, success, failure, progress) {
            ...
            xhr.onload = function() {
                ...
                success(pathJoin(settings.basePath, location));
            };
    ...
    var makeItems = function(info) {
        ...
        var imageDescription = {
            name: 'alt',
            type: 'input',
            label: tinymce.translate('ilimgupload.image_alt'),
    ...
    var maxSizeHintText =function (){
        ...
        if (hasAllowedFileExtensionText(editor)) {
            file_types = getAllowedFileExtensionsText(editor);
        }
        if (hasMaxUploadSizeText(editor)) {
            maxsize = getMaxUploadSizeText(editor);
        }
        return {
            maxsize: maxsize,
    ...
    var makeTab$2 = function(_info) {
        ...
        var items = [{
            type: 'dropzone',
            name: 'fileinput',
            label: tinymce.translate('ilimgupload.upload_image_from_local_fs_desc')
        }, {
            type: 'htmlpanel',
            html: '<p style="font-size:smaller">' + maxsize + '</p>' +
                '<p style="font-size:smaller">' + file_types + '</p>'
    ...
    var formFillFromMeta2 = function(info, data, meta) {
        ...
        if (info.hasAdvTab) {
            ...
            if (isString(meta.alignment)) {
                data.alignment = meta.alignment;
            }
        }
    };
    ...
    var changeStyle2 = function(parseStyle, serializeStyle, data) {
        ...
        dataCopy.alignment = calcAlignement(css);
        return dataCopy;
    };
    ...
    var submitHandler = function(editor) {
        return function(info) {
            return function(api) {
                var data = deepMerge(fromImageData(info.image), api.getData());
                editor.execCommand('mceUpdateilimgupload', false, toImageData(data, info.hasAccessibilityOptions));
    ...
    var register$1 = function(editor) {
        editor.ui.registry.addToggleButton('ilimgupload', {
            icon: 'image',
        ...
        editor.ui.registry.addMenuItem('ilimgupload', {
            icon: 'image',
            text: 'Image...',
            onAction: Dialog(editor).open
        });
        editor.ui.registry.addContextMenu('ilimgupload', {
            update: function(element) {
                return isFigure(element) || isImage(element) && !isPlaceholderImage(element) ? ['ilimgupload'] : [];
            }
        });
    };

    function Plugin() {
        global.add('ilimgupload', function(editor) {
            setup(editor);
            register$1(editor);
            //register$2(editor);
            register(editor);
