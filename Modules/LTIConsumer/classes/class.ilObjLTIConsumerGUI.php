...
class ilObjLTIConsumerGUI extends ilObject2GUI
{
	...
	protected function buildProviderSelectionForm($a_new_type)
	{
		return new ilLTIConsumerProviderSelectionFormTableGUI(
			$a_new_type, $this, 'create', 'applyProviderFilter', 'resetProviderFilter'
		);
	}
	...
	public static function _goto($a_target)
	{
		...
		{
			ilUtil::sendInfo(
				sprintf($DIC->language()->txt('msg_no_perm_read_item'),
		...
		switch( $DIC->ctrl()->getNextClass() )
		{
			...
			case strtolower(ilLearningProgressGUI::class):
				...
				$gui = new ilLearningProgressGUI(
					ilLearningProgressGUI::LP_CONTEXT_REPOSITORY, $this->object->getRefId()
				);
	...
	public function setTabs()
	{
		...
		$DIC->tabs()->addTab(self::TAB_ID_INFO,
		...
		{
			$DIC->tabs()->addTab(self::TAB_ID_CONTENT,
		...
		{
			$DIC->tabs()->addTab(self::TAB_ID_SETTINGS,
		...
		{
			$DIC->tabs()->addTab(self::TAB_ID_STATEMENTS,
        ...
        if( $this->ltiAccess->hasHighscoreAccess() ) {
            $DIC->language()->loadLanguageModule('lti');
            $DIC->tabs()->addTab(self::TAB_ID_SCORING,
		...
		{
			$DIC->tabs()->addTab(self::TAB_ID_LEARNING_PROGRESS,
		...
		if( $this->ltiAccess->hasWriteAccess() )
		{
			...
			if( strlen($link) )
			{
				$DIC->tabs()->addTab(self::TAB_ID_METADATA,
					$DIC->language()->txt('meta_data'), $link
				);
			}
		}
		...
		{
			$DIC->tabs()->addTab(self::TAB_ID_PERMISSIONS,
	...
	public function addLocatorItems()
	{
		...
		$locator->addItem($this->object->getTitle(),
			$this->ctrl->getLinkTarget($this, self::DEFAULT_CMD),"", $_GET["ref_id"]
		);
	}
	...
	protected function infoScreenForward()
	{
		...
		if( $this->object->getProvider()->getHasOutcome() ) {
            ...
            $info->addProperty($DIC->language()->txt("mastery_score"),
        ...
        $info->addProperty($DIC->language()->txt("lti_con_prov_url"),
		...
		{
			$info->addProperty($DIC->language()->txt("lti_info_external_provider_label"),
        ...
        {
            $info->addProperty($DIC->language()->txt("lti_con_prov_xapi_launch_url"),
	...
	protected function initInfoScreenToolbar()
	{
		...
		$token = ilCmiXapiAuthToken::fillToken(
			$DIC->user()->getId(), $this->object->getRefId(), $this->object->getId()
		);
		...
		$launchParameters = $this->object->buildLaunchParameters(
			$cmixUser, $token, $contextType, $contextId, $contextTitle, $returnUrl
		);
