...
class ilObjFileGUI extends ilObject2GUI
{
    ...
    public function saveUnzip()
    {
        ...
        if ($this->checkPermissionBool("create", "", "file")) {
            if ($zip_form_gui->checkInput()) {
                ...
                try {
                    $processDone = ilFileUtils::processZipFile(
                        $newDir,
                        $zip_file["tmp_name"],
     ...
     */
    public function confirmDeleteVersions()
    {
        global $DIC;
        $ilTabs = $DIC['ilTabs'];
     ...
     */
    public function cancelDeleteVersions()
    {
        $this->ctrl->redirect($this, "versions");
    }
     ...
     * @return
     */
    public function compareHistoryIds($v1, $v2)
    {
        if (is_array($v1)) {
