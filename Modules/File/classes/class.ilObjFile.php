...
class ilObjFile extends ilObject2 {
	...
	function createProperties($a_upload = false) {
		...
		$this->log->debug("ilObjFile::createProperties, ID: " . $this->getId() . ", Name: " . $this->getFileName() . ", Type: " . $this->getFileType()
		                  . ", Size: " . $this->getFileSize() . ", Mode: " . $this->getMode() . ", Name(Bytes): "
		                  . implode(":", ilStr::getBytesForString($this->getFileName())));
		$this->log->logStack(ilLogLevel::DEBUG);

		$q = "INSERT INTO file_data (file_id,file_name,file_type,file_size,version,f_mode) " . "VALUES (" . $ilDB->quote($this->getId(), 'integer')
		     . "," . $ilDB->quote($this->getFileName(), 'text') . "," . $ilDB->quote($this->getFileType(), 'text') . ","
		     . $ilDB->quote((int)$this->getFileSize(), 'integer') . "," . $ilDB->quote(1, 'integer') . "," . $ilDB->quote($this->getMode(), 'text')
		     . ")";
	...
	protected function doUpdate() {
		...
		$q = "UPDATE file_data SET file_name = " . $ilDB->quote($this->getFileName(), 'text') . ", file_type = "
		     . $ilDB->quote($this->getFiletype(), 'text') . " " . ", file_size = " . $ilDB->quote((int)$this->getFileSize(), 'integer') . " "
		     . ", version = " . $ilDB->quote($this->getVersion(), 'integer') . " " . ", f_mode = " . $ilDB->quote($this->getMode(), 'text') . " "
		     . ", rating = " . $ilDB->quote($this->hasRating(), 'integer') . " " . "WHERE file_id = " . $ilDB->quote($this->getId(), 'integer');
		$res = $ilDB->manipulate($q);
	...
	static function _writeFileType($a_id, $a_format) {
		...
		$q = "UPDATE file_data SET " . " file_type = " . $ilDB->quote($a_format, 'text') . " WHERE file_id = " . $ilDB->quote($a_id, 'integer');
		$res = $ilDB->manipulate($q);
	}
	 ...
	 */
	protected function guessFileType($a_file = "") {
		return ilMimeTypeUtil::lookupMimeType($a_file, $this->getFileType());
	}
	...
	protected function doCloneObject($a_new_obj, $a_target_id, $a_copy_id = 0) {
		...
		$query = "INSERT INTO file_data (file_id,file_name,file_type,file_size,version,rating,f_mode) VALUES ("
		         . $ilDB->quote($a_new_obj->getId(), 'integer') . "," . $ilDB->quote($this->getFileName(), 'text') . ","
		         . $ilDB->quote($this->getFileType(), 'text') . "," . $ilDB->quote((int)$this->getFileSize(), 'integer') . ", "
		         . $ilDB->quote($this->getVersion(), 'integer') . ", " . $ilDB->quote($this->hasRating(), 'integer') . ", "
	...
	static function _deleteAllUsages($a_type, $a_id, $a_usage_hist_nr = 0, $a_usage_lang = "-") {
		...
		$and_hist = ($a_usage_hist_nr !== false) ? " AND usage_hist_nr = " . $ilDB->quote($a_usage_hist_nr, "integer") : "";

		$file_ids = array();
		$set = $ilDB->query("SELECT id FROM file_usage" . " WHERE usage_type = " . $ilDB->quote($a_type, "text") . " AND usage_id= "
		                    . $ilDB->quote($a_id, "integer") . " AND usage_lang= " . $ilDB->quote($a_usage_lang, "text") . $and_hist);
		while ($row = $ilDB->fetchAssoc($set)) {
			$file_ids[] = $row["id"];
		}

		$ilDB->manipulate("DELETE FROM file_usage WHERE usage_type = " . $ilDB->quote($a_type, "text") . " AND usage_id = "
		                  . $ilDB->quote((int)$a_id, "integer") . " AND usage_lang= " . $ilDB->quote($a_usage_lang, "text") . " AND usage_hist_nr = "
	...
	static function _getFilesOfObject($a_type, $a_id, $a_usage_hist_nr = 0, $a_usage_lang = "-") {
		...
		// get usages in learning modules
		$q = "SELECT * FROM file_usage WHERE " . "usage_id = " . $ilDB->quote((int)$a_id, "integer") . " AND " . "usage_type = "
		     . $ilDB->quote((string)$a_type, "text") . " AND " . $lstr . "usage_hist_nr = " . $ilDB->quote((int)$a_usage_hist_nr, "integer");
	...
	public function rollback($version_id) {
		...
		include_once("./Services/History/classes/class.ilHistory.php");
		ilHistory::_createEntry($this->getId(), "rollback", $source["filename"] . "," . $new_version_nr . "|" . $source["version"] . "|"
		                                                    . $ilUser->getId());
	 ...
	 * @return int Returns an integer less than, equal to, or greater than zero if the first argument is considered to be respectively less than,
