    ...
    private static function recurseFolder($ref_id, $title, $tmpdir)
    {
        ...
        foreach ($subtree as $child) {
            ...
            if ($child["type"] == "fold") {
                ...
                // bugfix mantis 24309:
                // changed the code of this else-statement for allowing to check for duplicates
                // (identically named files) and renaming them to prevent an exception from occurring

                // enhance new_filename to full relative path to enable checking for identically named files
                $relative_path_tmp_dir = ilUtil::getDataDir() . "/temp/" . $tmpdir;
                $new_filename = $relative_path_tmp_dir . DIRECTORY_SEPARATOR . ilUtil::getASCIIFilename($child["title"]);

                // alter the filename if there are identically named files in the same folder
                if (file_exists($new_filename)) {
                    $new_filename = self::renameDuplicateFile($new_filename);
                }

                // sanitize the new filename to enforce ILIAS' file extension whitelist
                $sanitizer = new FilenameSanitizerImpl();
                $new_filename = $sanitizer->sanitize($new_filename);

                // copy to temporary directory
                $old_filename = ilObjFile::_lookupAbsolutePath($child["obj_id"]);
