...
class ilSCORMTrackingItems
{
    ...
    public function exportSelectedRaw($a_user = array(), $a_sco = array(), $b_orderBySCO=false, $allowExportPrivacy=false, $obj_id)
    {
        global $ilDB, $lng;
        $lng->loadLanguageModule("scormtrac");
...
//			. 'AND st.obj_id = '.$ilDB->quote($this->getId(),'integer') .' '
    ...
    */
    ...
    public function exportSelectedSuccessRows($a_user = array(), $allowExportPrivacy=false, $dbdata = array(), $scoCounter, $u_startedSCO, $u_completedSCO, $u_passedSCO, $obj_id)
    {
        $returnData=array();
        foreach ($dbdata as $data) {
            $dat=array();
