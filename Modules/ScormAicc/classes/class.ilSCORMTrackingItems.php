...
//		var_dump($returnData);
	...
	*
	*/
	function exportDataForImport($a_user = array()) {
		global $ilDB, $ilUser;
		...
		while ($data = $ilDB->fetchAssoc($res)) {
			$csv = $csv. $data["obj_id"]
				. ";\"" . $this->getTitle() ."\""
				. ";" . $data["module_version"]
				. ";\"" . implode("\";\"",self::userDataArrayForExport($data["user_id"], $allowExportPrivacy)) ."\""
				. ";\"" . $data["last_access"] ."\""
...
//				. ";\"" . $certificateDate ."\""
		...
		// $u_completedSCO = array();
		// $u_passedSCO = array();
		// $u_moduleVersion = array();
		// $u_packageAttempts = array();
		// for($i=0; $i<count($a_user); $i++) {
			// $u_lastAccess[$a_user[$i]] = '';
			// $u_startedSCO[$a_user[$i]] = 0;
			// $u_completedSCO[$a_user[$i]] = 0;
			// $u_passedSCO[$a_user[$i]] = 0;
			// $u_moduleVersion[$a_user[$i]] = '';
			// $u_packageAttempts[$a_user[$i]] = '';
		// }

		// $query = 'SELECT user_id, MAX(c_timestamp) exp_date , count(distinct(SCO_ID)) counter '
			// . 'FROM scorm_tracking ' 
			// . 'WHERE obj_id = %s '
			// . 'AND SCO_ID > 0 '
			// . 'AND '.$ilDB->in('user_id', $a_user, false, 'integer') .' '
		...
		// );
		// while ($data = $ilDB->fetchAssoc($res)) {
			// $u_lastAccess[$data['user_id']] = $data['exp_date'];
			// $u_startedSCO[$data['user_id']] = $data['counter'];
		// }
		...
		// $res = $ilDB->queryF(
			// $query,
			// array('integer','text','text'),
			// array($this->getId(),'cmi.core.lesson_status','completed')
		// );
		// while ($data = $ilDB->fetchAssoc($res)) {
			// $u_completedSCO[$data['user_id']] = $data['counter'];
		// }
		...
		// $res = $ilDB->queryF(
			// $query,
			// array('integer','text','text'),
			// array($this->getId(),'cmi.core.lesson_status','passed')
		// );
		// while ($data = $ilDB->fetchAssoc($res)) {
			// $u_passedSCO[$data['user_id']] = $data['counter'];
		// }

		// //too slow $this->getModuleVersionForUser($user)
		// $query = 'SELECT user_id, rvalue '
				// .'FROM scorm_tracking ' 
				// .'WHERE obj_id = %s AND lvalue = %s '
				// .'AND '.$ilDB->in('user_id', $a_user, false, 'integer') .' ';
		// $res = $ilDB->queryF(
			// $query,
			// array('integer','text'),
			// array($this->getId(),'module_version')
		// );
		// while ($data = $ilDB->fetchAssoc($res)) {
			// $u_moduleVersion[$data['user_id']] = $data['rvalue'];
		// }

		// //too slow $this->getAttemptsForUser($user)
		// $res = $ilDB->queryF(
			// $query,
			// array('integer','text'),
			// array($this->getId(),'package_attempts')
		// );
		// while ($data = $ilDB->fetchAssoc($res)) {
			// $u_packageAttempts[$data['user_id']] = $data['rvalue'];
		// }
		...
		// {
			// // get learning time
			// $time = 0;
			// $sco_set = $ilDB->queryF('SELECT rvalue FROM scorm_tracking '
				// .'WHERE obj_id = %s AND user_id = %s AND lvalue = %s',
				// array('integer','integer','text'),
				// array($this->getId(),$user, 'cmi.core.total_time'));

			// while($sco_rec = $ilDB->fetchAssoc($sco_set))
			// {
				// $time += $this->SCORMTimeToSeconds($sco_rec["rvalue"]);
			// }
			// //write export entry
			// // = ilUtil::stripScriptHTML($this->title);
			// $status = ilLearningProgressBaseGUI::__readStatus($this->getId(),$user);
			// $certificateDate = '';
			// if ($status == "trac_completed") {
// //				$date_completed= ilLPUserStatus::getCompletionDate($user,$this->getId());
// //				$certificateDate = strftime("%Y-%m-%d %H:%M:%S", $date_completed);
			// }
			// $csv = $csv. $this->getId()
				// . ";\"" . $this->title ."\""
				// . ";" . $u_moduleVersion[$user]
				// . ";" . $user;
			// if ($allowExportPrivacy == true) $csv .= $userString[$user];
			// $csv .= ";\"" . $u_lastAccess[$user] ."\""
				// . ";\"" . $status  ."\""
// //				. ";\"" . $certificateDate ."\""
				// . ";" . $u_packageAttempts[$user]
				// . ";" . $scoCounter
				// . ";" . $u_startedSCO[$user]
				// . ";" . $u_completedSCO[$user]
				// . ";" . $u_passedSCO[$user]
