...
function ilCloudFileList(url_get_block, url_create_folder, url_upload_file, url_delete_item, root_id, root_path, current_id, current_path, max_file_size_txt) {
      ...
      $.ajax({
        ...
        if (return_data.success) {
          ...
          self.showDebugMessage(
            "showCurrent: Block did not exist, not successfull ajax request. id=" + current_id + " path= " + current_path);

          if (return_data.message) {
            self.showDebugMessage(
              "showCurrent: Block did not exist, not successfull ajax request. message=" + return_data.message);
            self.showMessage(return_data.message);
          } else {
            self.showDebugMessage(
              "showCurrent: Block did not exist, not successfull ajax request. data=" + return_data);
  ...
  $.address.change(function (event) {
    if (event.pathNames[0] === "delete_item") {
      ...
      event.parameters.current_path ? current_path = decodeURIComponent((event.parameters.current_path + '').replace(
        /\+/g,
        '%20')) : current_path = current_path;
