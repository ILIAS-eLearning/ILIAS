<?php

require_once 'Services/Object/classes/class.ilObjectAccess.php';
require_once 'Services/AccessControl/classes/class.ilConditionHandler.php';
require_once 'Services/AccessControl/interfaces/interface.ilConditionHandling.php';
class ilObjManualAssessmentAccess extends ilObjectAccess implements ilConditionHandling{
	/**
	 * @inheritdoc
	 */
	static function _getCommands() {
		$commands = array(
			array("permission" => "read", "cmd" => "", "lang_var" => "show", "default" => true)
			,array("permission" => "write", "cmd" => "edit", "lang_var" => "edit", "default" => false)
		);
		return $commands;
	}

	/**
	 * ilConditionHandling implementation
	 *
	 * @inheritdoc
	 */
	public static function getConditionOperators() {
		include_once './Services/AccessControl/classes/class.ilConditionHandler.php';
		return array(
			ilConditionHandler::OPERATOR_PASSED,
			ilConditionHandler::OPERATOR_FAILED
		);
	}

	/**
	 * @inheritdoc
	 */
	public static function checkCondition($mass_id,$a_operator,$a_value,$a_usr_id) {
		require_once 'Modules/ManualAssessment/classes/LearningProgress/class.ilManualAssessmentLPInterface.php';
		switch($a_operator) {
			case ilConditionHandler::OPERATOR_PASSED:
				return ilManualAssessmentLPInterface::determineStatusOfMember($mass_id, $a_usr_id) 
					== ilManualAssessmentMembers::LP_COMPLETED;
				break;
			case ilConditionHandler::OPERATOR_FAILED:
				return ilManualAssessmentLPInterface::determineStatusOfMember($mass_id, $a_usr_id) 
					== ilManualAssessmentMembers::LP_FAILED;
			default:
				return false;
		}
		return false;
	}
}