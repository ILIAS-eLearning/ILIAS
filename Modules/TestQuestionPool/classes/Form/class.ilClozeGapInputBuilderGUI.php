...
class ilClozeGapInputBuilderGUI extends ilSubEnabledFormPropertyGUI
{
	...
	public function getValue()
	{
		...
		{
			$ilUtilObj = new ilUtil();
			$json=json_decode($ilUtilObj->stripSlashes($_POST['gap_json_post']));
			return $json[0];
		}
		return $this->value;
	}
	...
	public function getValueCombination()
	{
		...
		{
			$ilUtilObj = new ilUtil();
			$json=json_decode($ilUtilObj->stripSlashes($_POST['gap_json_combination_post' ]));
			return $json;
		}
	...
	public function checkInput()
	{
		...
		$error				= false;
		$json 				= json_decode($ilUtilObj->stripSlashes($_POST['gap_json_post']));
		$_POST['gap'] 		= $ilUtilObj->stripSlashesRecursive($_POST['gap']);
		if(array_key_exists('gap_combination',$_POST))
		{
			$best_solution = $ilUtilObj->stripSlashes($_POST['best_possible_solution']);
			if($best_solution == '')
			{
				$error = true;
			}
			$_POST['gap_combination'] = $ilUtilObj->stripSlashesRecursive($_POST['gap_combination']);
			...
			foreach($_POST['gap'] as $key => $item)
			{
				...
				if( $getType== CLOZE_TEXT || $getType == CLOZE_SELECT )
				{
					...
					if(array_key_exists('points',$gapText))
					{
						...
						{
							$_POST['shuffle_' . $key]= $ilUtilObj->stripSlashes($_POST['shuffle_' . $key]);
				...
				{
					$_POST['gap_' . $key .'numeric'] 		= $ilUtilObj->stripSlashes($_POST['gap_' . $key .'numeric'], FALSE);
					$_POST['gap_' . $key .'numeric_lower'] 	= $ilUtilObj->stripSlashes($_POST['gap_' . $key .'numeric_lower'], FALSE);
					$_POST['gap_' . $key .'numeric_upper'] 	= $ilUtilObj->stripSlashes($_POST['gap_' . $key .'numeric_upper'], FALSE);
					$_POST['gap_' . $key .'numeric_points'] = $ilUtilObj->stripSlashes($_POST['gap_' . $key .'numeric_points']);
