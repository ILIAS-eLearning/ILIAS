...
class ilAssLacExpressionNotSupportedByQuestion extends ilAssLacException implements ilAssLacFormAlertProvider
{
	...
	public function __construct($expression, $question_index)
	{
		...
		{
			$msg = sprintf(
				'The expression "%s" is not supported by the current question', $this->getExpression()
			);
		}
		else
		{
			$msg = sprintf(
				...
				$this->getExpression(), $this->getQuestionIndex()
			);
		}
	...
	public function getFormAlert(ilLanguage $lng)
	{
		...
		{
			return sprintf(
				$lng->txt("ass_lac_expression_not_supported_by_cur_question"), $this->getExpression()
			);
		}
		
		return sprintf(
			$lng->txt("ass_lac_expression_not_supported_by_question"), $this->getQuestionIndex(), $this->getExpression()
		);
	}
}