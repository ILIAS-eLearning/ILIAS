...
function addRow(e, obj)
{
	...
	addbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'singlechoicewizard_add') ? true : false; }, 'input', rowclone);
	for (i = 0; i < addbuttons.length; i++)
	{
		YAHOO.util.Event.addListener(addbuttons[i], 'click', addRow);
	}
	removebuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'singlechoicewizard_remove') ? true : false; }, 'input', rowclone);
	for (i = 0; i < removebuttons.length; i++)
	{
		YAHOO.util.Event.addListener(removebuttons[i], 'click', removeRow);
	}
	upbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'singlechoicewizard_up') ? true : false; }, 'input', rowclone);
	for (i = 0; i < upbuttons.length; i++)
	{
		YAHOO.util.Event.addListener(upbuttons[i], 'click', moveRowUp);
	}
	downbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'singlechoicewizard_down') ? true : false; }, 'input', rowclone);
	for (i = 0; i < downbuttons.length; i++)
	{
		YAHOO.util.Event.addListener(downbuttons[i], 'click', moveRowDown);
	}
