...
function reindexRows(rootel, postvar)
{
	...
	for (i = 0; i < rows.length; i++)
	{
		...
		addbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_add') ? true : false; }, 'input', rows[i]);
		...
		removebuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_remove') ? true : false; }, 'input', rows[i]);
		...
		upbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_up') ? true : false; }, 'input', rows[i]);
		...
		downbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_down') ? true : false; }, 'input', rows[i]);
...
{
	var addbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_add') ? true : false; }, 'input', rootel);
	for (i = 0; i < addbuttons.length; i++)
	{
		YAHOO.util.Event.purgeElement(addbuttons[i]);
	}
	var removebuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_remove') ? true : false; }, 'input', rootel);
	for (i = 0; i < removebuttons.length; i++)
	{
		YAHOO.util.Event.purgeElement(removebuttons[i]);
	}
	var upbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_up') ? true : false; }, 'input', rootel);
	for (i = 0; i < upbuttons.length; i++)
	{
		YAHOO.util.Event.purgeElement(upbuttons[i]);
	}
	var downbuttons = YAHOO.util.Dom.getElementsBy(function (el) { return (el.className == 'matchingwizard_down') ? true : false; }, 'input', rootel);
...
function matchingWizardEvents(e)
{
	addbuttons = YAHOO.util.Dom.getElementsByClassName('matchingwizard_add');
	for (i = 0; i < addbuttons.length; i++)
	{
		button = addbuttons[i];
		YAHOO.util.Event.addListener(button, 'click', addRow);
	}
	removebuttons = YAHOO.util.Dom.getElementsByClassName('matchingwizard_remove');
	for (i = 0; i < removebuttons.length; i++)
	{
		button = removebuttons[i];
		YAHOO.util.Event.addListener(button, 'click', removeRow);
	}
	upbuttons = YAHOO.util.Dom.getElementsByClassName('matchingwizard_up');
	for (i = 0; i < upbuttons.length; i++)
	{
		button = upbuttons[i];
		YAHOO.util.Event.addListener(button, 'click', moveRowUp);
	}
	downbuttons = YAHOO.util.Dom.getElementsByClassName('matchingwizard_down');
	for (i = 0; i < downbuttons.length; i++)
	{
...
YAHOO.util.Event.onDOMReady(matchingWizardEvents);