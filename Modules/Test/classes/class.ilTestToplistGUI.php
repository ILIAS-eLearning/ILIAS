...
class ilTestToplistGUI
{
    ...
    protected function manageTabs(ilTabsGUI $tabsGUI, ilCtrl $ctrl, ilLanguage $lng, $activeTabId)
    {
        ...
        $tabsGUI->setBackTarget(
            $lng->txt('tst_results_back_introduction'),
            $ctrl->getLinkTargetByClass('ilObjTestGUI', 'infoScreen')
        );

        $tabsGUI->addTab(
            'toplist_by_score',
            $lng->txt('toplist_by_score'),
            $ctrl->getLinkTarget($this, 'showResultsToplistByScore')
        );
        
        $tabsGUI->addTab(
            'toplist_by_time',
            $lng->txt('toplist_by_time'),
            $ctrl->getLinkTarget($this, 'showResultsToplistByTime')
        );
