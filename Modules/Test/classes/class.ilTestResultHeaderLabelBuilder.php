...
class ilTestResultHeaderLabelBuilder
{
	...
	public function getPassOverviewHeaderLabel()
	{
		...
		{
			return sprintf(
				$this->lng->txt('tst_pass_overview_header_lo_initial_all_objectives'), $this->crsTitle
			);
		}
		elseif( $this->isInitialTestPerObjective() )
		{
			return sprintf(
				...
				$this->getObjectivesString(), $this->crsTitle
			);
		}
		elseif( $this->isQualifyingTestForAllObjectives() )
		{
			return sprintf(
				$this->lng->txt('tst_pass_overview_header_lo_qualifying_all_objectives'), $this->crsTitle
			);
		}
	...
	public function getPassDetailsHeaderLabel($attemptNumber)
	{
		...
		if( $this->isInitialTest() )
		{
			return sprintf(
				...
				$this->getObjectivesString(), $this->getAttemptLabel($attemptNumber)
			);
		}
	...
	public function getVirtualPassDetailsHeaderLabel($objectiveTitle)
	{
		...
		{
			return sprintf(
				$this->lng->txt('tst_virtual_pass_header_lo_initial'), $objectiveTitle
			);
		}
		elseif( $this->isQualifyingTest() )
		{
			return sprintf(
				$this->lng->txt('tst_virtual_pass_header_lo_qualifying'), $objectiveTitle
			);
		}
