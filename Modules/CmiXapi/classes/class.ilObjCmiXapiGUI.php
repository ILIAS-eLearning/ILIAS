...
class ilObjCmiXapiGUI extends ilObject2GUI
{
	...
	protected function afterSave(ilObject $newObject)
	{
		...
		if( $form->checkInput() )
		{
			...
			$newObject->setUserIdent($newObject->getLrsType()->getUserIdent());
			$newObject->setUserName($newObject->getLrsType()->getUserName());
	...
	protected function setTabs()
	{
		...
		if( $this->cmixAccess->hasWriteAccess() )
		{
			$DIC->tabs()->addTab(self::TAB_ID_EXPORT,
	...
	function infoScreenForward()
	{
        ...
        {
            $info->addProperty($DIC->language()->txt("cmix_info_external_lrs_label"),
		...
		{
			$info->addProperty($DIC->language()->txt("cmix_indication_to_user"),
