	...
	protected function render($top = true)
	{
		...
		if(!$this->lm_tree->isInTree($page_id))
		{
			...
			if (!$this->lm->cleanFrames())
			{
				...
				$back_href =
					$this->parent_gui->getLink($this->lm->getRefId(), "layout", $back_pg, $frame,
						"", "reduce");
				$back_target = "";
			}
			else
			{
				$back_href =
					$this->parent_gui->getLink($this->lm->getRefId(), "layout", $back_pg, "",
						"", "reduce");
