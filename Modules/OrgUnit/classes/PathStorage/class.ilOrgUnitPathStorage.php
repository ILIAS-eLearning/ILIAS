...
class ilOrgUnitPathStorage extends ActiveRecord {
	...
	public static function getTextRepresentationOfUsersOrgUnits($user_id, $separator = self::ORG_SEPARATOR, $using_tmp_table = true) {
		if ($using_tmp_table) {
			...
			$res = $ilDB->queryF("SELECT " . $ilDB->loadModule('Manager')->getQueryUtils()->stringAgg("path", $separator) . " AS orgus FROM orgu_usr_assignements WHERE user_id = %s GROUP BY user_id;", array( 'integer' ), array( $user_id ));
