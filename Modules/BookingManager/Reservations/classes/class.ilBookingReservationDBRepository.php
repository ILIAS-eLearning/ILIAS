...
class ilBookingReservationDBRepository
{
	 ...
	 * @return int
	 */
	public function create(int $user_id, int $assigner_id, int $object_id,
		$context_obj_id,
		int $from,
		int $to,
		$status,
	 ...
	 * @return int
	 */
	public function update(int $id, int $user_id, int $assigner_id, int $object_id,
		$context_obj_id,
		int $from,
		int $to,
		$status,
	...
	public function getNumberOfReservations(array $ids, int $from, int $to, $only_not_over_yet = false)
	{
		...
		$set = $ilDB->query('SELECT count(*) cnt, object_id' .
			...
			' AND (status IS NULL OR status <> ' . $ilDB->quote(ilBookingReservation::STATUS_CANCELLED,
				'integer') . ')' .
	 ...
	 * @return	array
	 */
	function getListByDate($a_has_schedule, array $a_object_ids = null, array $filter = null,
