...
class ilBookingReservationsTableGUI extends ilTable2GUI
{
    ...
    {
        return $this->pool->usesMessages() && $this->access->checkAccess('write', '', $this->ref_id);
    }
    ...
    public function getItems(array $filter): void
    {
        ...
        if (count($this->getSelectedUserColumns()) > 0) {
            ...
            if ($odf_ids !== []) {
                $parent_obj_id = ilObject::_lookupObjectId($this->getParentGroupCourse()['ref_id']);
                $parent_obj_type = $this->getParentGroupCourse()['type'];
