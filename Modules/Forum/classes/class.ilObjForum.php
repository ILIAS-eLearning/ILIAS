...
class ilObjForum extends ilObject
{
    ...
    public function cloneObject(int $target_id, int $copy_id = 0, bool $omit_tree = false): ?ilObject
    {
        ...
        if (
            ...
            $new_obj->updateUserNotifications($object_properties->getNotificationType());
        }
    ...
    public function updateUserNotifications(string $notification_type): void
    {
        $object_properties = ilForumProperties::getInstance($this->getId());
        if (!in_array($notification_type, ['all_users', 'per_user'])) {
            return;
        }
        $oParticipants = $this->getParticipants();

        $frm_noti = new ilForumNotification($this->getRefId());
        ...
        foreach ($all_forum_users as $user_id) {
            ...
            $frm_noti->setUserToggle($object_properties->isUserToggleNoti());
            $frm_noti->setInterestedEvents($object_properties->getInterestedEvents());

            if (array_key_exists($user_id, $all_notis) && $notification_type === 'all_users') {
                $frm_noti->update();
    ...
    public function getParticipants(): ilParticipants
    {
        ...
        $crs_ref_id = $this->tree->checkForParentType($this->getRefId(), 'crs');
    ...
    {
        $grpRefId = $this->tree->checkForParentType($this->getRefId(), 'grp');
        $crsRefId = $this->tree->checkForParentType($this->getRefId(), 'crs');
