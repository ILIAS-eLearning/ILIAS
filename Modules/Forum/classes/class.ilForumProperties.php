...
class ilForumProperties
{
	...
	private function read()
	{
		if ($this->obj_id)
		{
			...
			if (is_object($row))
			{
				...
				$this->admin_force_noti = $row->admin_force_noti;
				$this->user_toggle_noti = $row->user_toggle_noti;
	...
	public function insert()
	{
		if ($this->obj_id)
		{
			...
			array($this->obj_id, $this->default_view, $this->anonymized,
			$this->statistics_enabled, $this->post_activation_enabled,
			$this->isAdminForceNoti(),$this->isUserToggleNoti()));
	...
	public function update()
	{
		if ($this->obj_id)
		{
			$statement = $this->db->manipulateF('UPDATE frm_settings
				...
				WHERE obj_id = %s', 
			array ('integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer'),
			array($this->default_view, $this->anonymized,
			$this->statistics_enabled, $this->post_activation_enabled,
			$this->isAdminForceNoti(),$this->isUserToggleNoti(),$this->obj_id));
	...
	public function copy($a_new_obj_id)
	{
		if ($a_new_obj_id)
		{
			$statement = $this->db->manipulateF('INSERT INTO frm_settings
				...
				VALUES( %s, %s, %s, %s, %s, %s, %s)',
			array ('integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer'),
			array($a_new_obj_id, $this->default_view, $this->anonymized, $this->statistics_enabled,
			$this->post_activation_enabled,$this->isAdminForceNoti(),$this->isUserToggleNoti()));
	...
	{
		$this->admin_force_noti = (int)$a_admin_force;
	}
	...
	{	
		$this->user_toggle_noti = (int)$a_user_toggle;
	}
	...
	static function _isAdminForceNoti($a_obj_id)
	{
		...
		$result = $ilDB->queryF("SELECT admin_force_noti FROM frm_settings WHERE obj_id = %s",
		array('integer'),array($a_obj_id));
		while($record = $ilDB->fetchAssoc($result))
		{
	...
	static function _isUserToggleNoti($a_obj_id)
	{
		...
		$result = $ilDB->queryF("SELECT user_toggle_noti FROM frm_settings WHERE obj_id = %s",
		array('integer'),array($a_obj_id));
		while($record = $ilDB->fetchAssoc($result))
		{
