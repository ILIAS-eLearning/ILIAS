...
class ilPCPRGActionNote extends ilPageContent
{
    protected ilLanguage $lng;
    public const PCTYPE = 'prgactionnote';
    public const PCELEMENT = 'PRGActionNote';
    public const PLACEHOLDER = '[[[PRG_ACTION_NOTE]]]';
    ...
    public function modifyPageContentPostXsl(
        ...
        $end = 0;
        $start = strpos($a_output, "[[[PRG_ACTION_NOTE");
        ...
        if ($a_mode === 'edit') {
            while ($end > 0) {
                if ($this->supportsType($parent_obj_id)) {
                    ...
                    $start = strpos($a_output, "[[[PRG_ACTION_NOTE", $start + 3);
                    $end = 0;
                    if (is_int($start)) {
                        $end = strpos($a_output, "]]]", $start);
                    }
    ...
    {
        $parent_obj_id = ilObject::_lookupType($parent_obj_id);
        return in_array($parent_obj_id, self::PROVIDING_TYPES);
    }

    protected function replaceWithRendered(int $obj_id, $html): string
    {
        $dic = ilStudyProgrammeDIC::dic();
        $builder = $dic['pc.actionnote'];
        $rendered = $builder->getNoteFor($obj_id);
        return str_replace(self::PLACEHOLDER, $rendered, $html);
    }

    protected function getTemplate(): string
    {
        $template = new ilTemplate("tpl.action_note_poeditor_element.html", true, true, 'Modules/StudyProgramme');
        ...
        $template->setVariable("LABEL", $this->lng->txt("pc_prg_action_note_label"));
