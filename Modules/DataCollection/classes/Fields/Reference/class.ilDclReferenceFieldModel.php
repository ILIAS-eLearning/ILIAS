...
class ilDclReferenceFieldModel extends ilDclBaseFieldModel {
	...
	public function getRecordQuerySortObject($direction = "asc", $sort_by_status = false){
		...
		$select_str = ($n_ref) ? " ". $ilDB->loadModule('Manager')->getQueryUtils()->stringAgg("stloc_{$this->getId()}_joined.value") . " AS field_{$this->getId()}" : "stloc_{$this->getId()}_joined.value AS field_{$this->getId()},";
