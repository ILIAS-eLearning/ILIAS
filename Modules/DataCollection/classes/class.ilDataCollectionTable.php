...
class ilDataCollectionTable {
	...
	public function doCreate() {
		...
		$query = "INSERT INTO il_dcl_table (" . "id" . ", obj_id" . ", title" . ", add_perm" . ", edit_perm" . ", delete_perm" . ", edit_by_owner"
			. ", limited" . ", limit_start" . ", limit_end" . ", is_visible" . ", export_enabled" . ", description" . " ) VALUES ("
	...
	public function hasPermissionToAddRecord($ref) {
		return ($this->getAddPerm() && ilObjDataCollection::_hasReadAccess($ref) && $this->checkLimit())
		|| ilObjDataCollection::_hasWriteAccess($ref);
	}
	...
	public function hasPermissionToEditRecord($ref, $record) {
		return ($this->getEditPerm() && ilObjDataCollection::_hasReadAccess($ref) && $this->checkEditByOwner($record) && $this->checkLimit())
		|| ilObjDataCollection::_hasWriteAccess($ref);
	}
	...
	public function hasPermissionToDeleteRecord($ref, $record) {
		return ($this->getDeletePerm() && ilObjDataCollection::_hasReadAccess($ref) && $this->checkEditByOwner($record) && $this->checkLimit())
		|| ilObjDataCollection::_hasWriteAccess($ref);
	}
			...
			switch ($sortField->getDatatypeId()) {
				...
				case ilDataCollectionDatatype::INPUTFORMAT_ILIAS_REF:
					$joinStr .=
						...
						. $ilDB->quote($sortField->getId(), 'integer') . ") ";
					...
					if ($sortByStatus) {
						global $ilUser;
						$joinStr .= "LEFT JOIN ut_lp_marks AS ut ON (ut.obj_id = sort_object_data_{$id}.obj_id AND ut.usr_id = "
				...
				switch ($filterField->getDatatypeId()) {
					...
					case ilDataCollectionDatatype::INPUTFORMAT_DATETIME:
						...
						if ($filterField->isStandardField()) {
							if ($dateFrom) {
								$where_additions .= " AND (record.{$filter_field_id} >= " . $ilDB->quote($dateFrom, 'date') . ")";
							}
							if ($dateTo) {
								$where_additions .= " AND (record.{$filter_field_id} <= " . $ilDB->quote($dateTo, 'date') . ")";
							}
						} else {
							$joinStr .=
								"INNER JOIN il_dcl_record_field AS filter_record_field_{$filter_field_id} ON (filter_record_field_{$filter_field_id}.record_id = record.id AND filter_record_field_{$filter_field_id}.field_id = "
							...
							$joinStr .= "INNER JOIN il_dcl_stloc{$filterField->getStorageLocation()}_value AS filter_stloc_{$filter_field_id} ON (filter_stloc_{$filter_field_id}.record_field_id = filter_record_field_{$filter_field_id}.id ";
							if ($dateFrom) {
								$joinStr .= "AND filter_stloc_{$filter_field_id}.value >= " . $ilDB->quote($dateFrom, 'date') . " ";
							}
							if ($dateTo) {
								$joinStr .= "AND filter_stloc_{$filter_field_id}.value <= " . $ilDB->quote($dateTo, 'date') . " ";
							}
							$joinStr .= ") ";
						}
						break;
					case ilDataCollectionDatatype::INPUTFORMAT_NUMBER:
						...
						if ($filterField->isStandardField()) {
							...
							$joinStr .= "INNER JOIN il_dcl_stloc{$filterField->getStorageLocation()}_value AS filter_stloc_{$filter_field_id} ON (filter_stloc_{$filter_field_id}.record_field_id = filter_record_field_{$filter_field_id}.id";
							if (! is_null($from)) {
								$joinStr .= " AND filter_stloc_{$filter_field_id}.value >= " . $ilDB->quote($from, 'integer');
							}
							if (! is_null($to)) {
								$joinStr .= " AND filter_stloc_{$filter_field_id}.value <= " . $ilDB->quote($to, 'integer');
							}
							$joinStr .= ") ";
						}
						break;
					...
					case ilDataCollectionDatatype::INPUTFORMAT_TEXT:
						if ($filterField->isStandardField()) {
							...
							$joinStr .=
								...
								. $ilDB->quote($filter_field_id, 'integer') . ") ";
							$joinStr .=
								" INNER JOIN il_dcl_stloc{$filterField->getStorageLocation()}_value AS filter_stloc_{$filter_field_id} ON (filter_stloc_{$filter_field_id}.record_field_id = filter_record_field_{$filter_field_id}.id AND filter_stloc_{$filter_field_id}.value LIKE "
								. $ilDB->quote("%$filter_value%", 'text') . ") ";
						}
						break;
					case ilDataCollectionDatatype::INPUTFORMAT_REFERENCE:
						...
						if ($nRef) {
							...
							$joinStr .=
								" INNER JOIN il_dcl_stloc{$filterField->getStorageLocation()}_value AS filter_stloc_{$filter_field_id} ON (filter_stloc_{$filter_field_id}.record_field_id = filter_record_field_{$filter_field_id}.id AND filter_stloc_{$filter_field_id}.value = "
