...
//				xhttp.send(data ? String(data) : '');				
	...
	if (r.content) {
		if (r.content.indexOf("login.php")>-1) {
			window.location.href = "./components/ILIAS/Scorm2004_/templates/default/session_timeout.html";
		}
	}
...
function IliasLaunch(i_l){
	...
	if (asset==1 || Initialized==false){
		...
		iv.launchId=i_l;
		frames.sahs_content.document.location.replace('./components/ILIAS/ScormAicc_/templates/default/dummy.html');
	}
