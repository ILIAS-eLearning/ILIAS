...
class AgentDetermination
{
    ...
    public function __construct()
    {
        ...
        */
        if (preg_match('/MSIE (\d.\d{1,2})/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = 'IE';
        } elseif (preg_match('/Opera (\d.\d{1,2})/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/Opera\/(\d.\d{1,2})/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = 'OPERA';
        } elseif (preg_match('/Safari ([0-9\/.]*)/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/Safari\/([0-9\/.]*)/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = 'Safari';
        } elseif (preg_match('/Firefox ([0-9\/.]*)/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/Firefox\/([0-9\/.]*)/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = 'Firefox';
        } elseif (preg_match('/iCab (\d.[0-9a-zA-Z]{1,4})/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/iCab\/(\d.[0-9a-zA-Z]{1,4})/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = 'iCab';
        } elseif (preg_match('/Mozilla (\d.[0-9a-zA-Z]{1,4})/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/Mozilla\/(\d.[0-9a-zA-Z]{1,4})/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
            $this->agent_name = preg_match('/Gecko/', (string) $HTTP_USER_AGENT, $log_version) ? 'Mozilla' : 'Netscape';
        } elseif (preg_match('/Konqueror\/(\d.\d.\d{1,2})/', (string) $HTTP_USER_AGENT, $log_version) ||
            preg_match('/Konqueror\/(\d.\d{1,2})/', (string) $HTTP_USER_AGENT, $log_version)) {
            $this->agent_version = $log_version[1];
        ...
        /*
            Determine platform
        */
        if (strstr((string) $HTTP_USER_AGENT, 'Win')) {
            $this->agent_platform = 'Win';
        } elseif (strstr((string) $HTTP_USER_AGENT, 'Mac')) {
            $this->agent_platform = 'Mac';
        } elseif (strstr((string) $HTTP_USER_AGENT, 'Linux')) {
            $this->agent_platform = 'Linux';
        } elseif (strstr((string) $HTTP_USER_AGENT, 'Unix')) {
            $this->agent_platform = 'Unix';
        } elseif (strstr((string) $HTTP_USER_AGENT, 'Beos')) {
        ...
        if (isset($_SERVER['HTTP_X_WAP_PROFILE']) || isset($_SERVER['HTTP_PROFILE'])) {
            $this->is_mobile = true;
        } elseif (strpos((string) $this->accept, 'text/vnd.wap.wml') > 0 || strpos(
            (string) $this->accept,
