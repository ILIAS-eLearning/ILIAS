...
class ilContainerXmlParser
{
    ...
    protected function initItem(
        ...
        $new_ref = $this->getMapping()->getMapping('components/ILIAS/Container_', 'refs', $ref_id);
        ...
        if (
            ...
            $new_ref = $this->mapping->getMapping('components/ILIAS/Container_', 'refs', '0');

            // see below and ilContainerImporter::finalProcessing()
            $this->mapping->addMapping('components/ILIAS/Container_', 'objs', $obj_id, (string) ilObject::_lookupObjId((int) $new_ref));
            $this->mapping->addMapping('components/ILIAS/Object_', 'obj', $obj_id, (string) ilObject::_lookupObjId((int) $new_ref));
        }
        ...
        $new_obj_id = $this->mapping->getMapping('components/ILIAS/Container_', 'objs', $obj_id);
        ...
        if ($ilSetting->get('enable_cat_page_edit', '0')) {
            if ($item['Page'] == "1") {
                $this->mapping->addMapping('components/ILIAS/COPage_', 'pg', 'cont:' . $obj_id, 'cont:' . $new_obj_id);
                $this->cont_log->debug("add pg cont mapping, old: " . $obj_id . ", new: " . $new_obj_id . ", Page: -" . $item['Page'] . "-");
            }

            if ($item['StartPage'] == "1") {
                $this->mapping->addMapping('components/ILIAS/COPage_', 'pg', 'cstr:' . $obj_id, 'cstr:' . $new_obj_id);
            }
        }
    }
    ...
    protected function createObject(
        ...
        $new_obj_id = $this->getMapping()->getMapping('components/ILIAS/Container_', 'objs', (string) $obj_id);
        if ($new_obj_id) {
            ...
            if ($obj instanceof  ilObject) {
                ...
                $this->mapping->addMapping('components/ILIAS/Container_', 'refs', (string) $ref_id, (string) $obj->getRefId());
                return $obj->getRefId();
            }
        }
        ...
        $this->mapping->addMapping('components/ILIAS/Container_', 'objs', (string) $obj_id, (string) $new->getId());
        $this->mapping->addMapping('components/ILIAS/Object_', 'obj', (string) $obj_id, (string) $new->getId());
        $this->mapping->addMapping('components/ILIAS/Container_', 'refs', (string) $ref_id, (string) $new->getRefId());
