    ...
    public function importObject(
        ...
        if ($error === '') {
            ...
            if (strpos($xml, "questestinterop")) {
                ...
                $imp = new ilImport();
                $config = $imp->getConfig("components/ILIAS/Survey_");
                $config->setQuestionPoolID($svy_qpl_id);
                $imp->getMapping()->addMapping("components/ILIAS/Survey_", "svy", 0, $this->getId());
                $imp->importFromDirectory($import_subdir, "svy", "components/ILIAS/Survey_");
