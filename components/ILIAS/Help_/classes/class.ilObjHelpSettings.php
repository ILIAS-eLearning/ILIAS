...
class ilObjHelpSettings extends ilObject2
{
    ...
    public function uploadHelpModule(
        ...
        try {
            $imp = new ilImport();
            $conf = $imp->getConfig("components/ILIAS/Help_");
            $conf->setModuleId($id);
            $new_id = $imp->importObject(null, $a_file["tmp_name"], $a_file["name"], "lm", "components/ILIAS/LearningModule_"); //
