...
abstract class DocumentProcessorBase implements DocumentProcessor
{
    public function parseInstructionsToAssocArray(string $a_raw_mount_instructions): array
    {
        ...
        do {
            ...
            if ($open_tag_start_pos !== false && $open_tag_end_pos !== false && $open_tag_start_pos < $open_tag_end_pos) {
                $tag_name = substr(
                    $processing_text,
                    $open_tag_start_pos + 1,
                    $open_tag_end_pos - $open_tag_start_pos - 1
                );
                $close_tag = "[/$tag_name]";
                ...
                if ($close_tag_pos !== false && $open_tag_end_pos < $close_tag_pos) {
                    $found_instructions[$tag_name] = substr(
                        $processing_text,
                        $open_tag_end_pos + 1,
                        $close_tag_pos - $open_tag_end_pos - 1
                    );
