...
class DocumentTableGUI extends ilTable2GUI
{
    ...
    public function __construct(
        UploadGUI $parent_obj,
        protected UriBuilder $webdav_uri_builder,
        string $command,
    ...
    public function setProvider(TableDataProvider $provider): void
    {
        $this->provider = $provider;
    }

    public function getProvider(): ?TableDataProvider
    {
    ...
    public function getSelectableColumns(): array
    {
        $optional_columns = array_filter(
            ...
            static fn(array $column): bool => isset($column['optional']) && $column['optional']
        );
    ...
    protected function formatCellValue(string $column, array $row): string
    {
        ...
        if (in_array($column, ['creation_ts', 'modification_ts'], true)) {
    ...
    protected function formatTitle(string $column, array $row): string
    {
        ...
        $url = $this->webdav_uri_builder->getUriToMountInstructionModalByLanguage($row['language']);
    ...
    {
        $value = (string) (($this->i++) * $this->factor);
        if (!$this->is_editable) {
            return $value;
        }
