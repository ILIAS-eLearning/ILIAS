...
class InstructionsGUI
{
    public function __construct(
        protected BaseInstructions $mount_instruction,
    ...
    public function buildGUIFromGivenMountInstructions(array $mount_instructions, bool $render_async = false): string
    {
        ...
        foreach ($mount_instructions as $title => $text) {
            foreach ($os as $os_string) {
                if (stripos((string) $title, (string) $os_string) !== false) {
                    $selected = $title;
                    break 2;
                }
            }
        ...
        $view_control = $f->viewControl()->mode($view_control_actions, 'mount-instruction-buttons')->withActive(
            $selected
        );
    ...
    public function renderMountInstructionsContent(): void
    {
        try {
            ...
            $document_processor = new HtmlDocumentProcessor(
                new DocumentPurifier()
            );
            $instructions = $document_processor->processMountInstructions(
                $this->lang->txt('webfolder_instructions_text')
            );
            $instructions = $this->mount_instruction->getMountInstructionsAsArray($instructions);
            if ($instructions === '-webfolder_instructions_text-') {
                $instructions = [
                    "<div class='alert alert-danger'>" . $this->lang->txt('error')
                        . ': ' . $this->lang->txt('webdav_missing_lang') . '</div>',
                ];
            }
        }
    ...
    private function determineOSfromUserAgent(): array
    {
        ...
        if (stripos($ua, 'windows') !== false || str_contains($ua, 'microsoft')) {
            return ['win'];
        }

        if (stripos($ua, 'darwin') !== false || stripos($ua, 'macintosh') !== false) {
            return ['mac', 'osx'];
        }

        if (stripos($ua, 'linux') !== false
            || stripos($ua, 'solaris') !== false
            || stripos($ua, 'aix') !== false
            || stripos($ua, 'unix') !== false
            || stripos($ua, 'gvfs') !== false // nautilus browser uses this ID
        ) {
            return ['linux'];
        }
