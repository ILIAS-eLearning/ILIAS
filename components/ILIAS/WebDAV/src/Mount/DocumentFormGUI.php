...
class DocumentFormGUI extends ilPropertyFormGUI
{
    ...
    protected function createFilledObject(Document $document): Document
    {
        if (!$this->checkInput()) {
        ...
        if ($document_already_exists) {
            ...
            $document_processor = $upload_result->getMimeType() === 'text/html'
                ? new HtmlDocumentProcessor($this->document_purifier)
