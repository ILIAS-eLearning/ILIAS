...
class TableColumnContextRenderer extends Renderer
{
    ...
    protected function renderList(Ordered|Unordered $component, RendererInterface $default_renderer): string
    {
        if (self::LIST_DISPLAY_LIMIT >= count($component->getItems())) {
        ...
        $template->setVariable('SHOW_MORE_LABEL', sprintf(
            $this->txt('show_x_items'),
            count($component->getItems()) - self::LIST_DISPLAY_LIMIT
        ));
        ...
        $this->toJS('show_x_items');
        $this->toJS('hide_x_items');
