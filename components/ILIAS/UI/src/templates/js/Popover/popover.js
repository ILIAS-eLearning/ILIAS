...
(function ($, UI) {
  UI.popover = (function ($) {
    ...
    /**
     * Show a popover for a triggerer element (the element triggering the show signal) with the
     * given options.
     *
    ...
    /**
     * Replace the content of the popover showed by the given showSignal with the data returned
    ...
    const replaceContentFromSignal = function (showSignal, signalData) {
      ...
      il.UI.core.AsyncRenderer.replaceContent(url, contentElement);
    };
    ...
    var show = function ($triggerer, options) {
      ...
      // popovers in forms without specifying a container here. We search for upper
      // il-popover-container. If given we use this element as a container for the popover
      let container;
