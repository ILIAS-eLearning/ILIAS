 ...
 * ---
 * description: >
 *   An example showing a Multi-Select Input with an Option Filter and async data source
 *
 * expected output: >
 *   A Multi-Select with an Option Filter that can be expanded, collapsed and filtered.
 *   When expanded, there is a search input field.
 ...
 *   A clear filter button resets the search input fields.
...
function with_has_option_filter_data_source(): string
{
    ...
    $ctrl->setParameterByClass(ilSystemStyleDocumentationGUI::class, 'node_id', 'InputFieldMultiSelectMultiSelect');
    $async_select = $ui->input()->field()->multiSelect(
        "Role",
        [],
        "Select multiple roles provided by a data source"
    )
        ->withHasOptionFilter(true, $ctrl->getLinkTargetByClass(ilSystemStyleDocumentationGUI::class, 'entries', '', true));
    $async_select = $async_select->withValue(["admin", "custom1"]);

    //Step 4: define form and form actions
    $form = $ui->input()->container()->form()->standard('#', ['select' => $async_select]);
