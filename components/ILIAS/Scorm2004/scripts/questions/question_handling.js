...
const ilias = {}; // namespace
let icount = 0; // interaction count
ilias.UTILS = {};
ilias.questions = {
  answers: Object,
...
ilias.questions.checkAnswers = function (a_id) {
  ...
  const call = `ilias.questions.${questions[a_id].type}(${a_id})`;
...
ilias.questions.handleMCImages = function (a_id) {
  ...
  jQuery(`div#container${a_id} input.order`).each((key, node) => {
    for (let i = 0; i < questions[a_id].answers.length; i++) {
      if (questions[a_id].answers[i].order == node.value) {
        const img = questions[a_id].answers[i].image;
        if (img.length) {
          const text_node = jQuery(node).closest('.ilc_qanswer_Answer').find('.answertext');
...
ilias.questions.handleKprimImages = function (a_id) {
  ...
  jQuery(`div#container${a_id} .answers`).each(function () {
    const $firstOption = jQuery(this).find('input[type="radio"]:first');
    for (let i = 0; i < questions[a_id].answers.length; i++) {
      if (`kprim_choice_${a_id}_result_${questions[a_id].answers[i].order}` == $firstOption.prop('name')) {
        const img = questions[a_id].answers[i].image;
        if (img.length) {
          const text_node = $firstOption.closest('tr').find('.answertext');
          if (questions[a_id].thumb > 0) {
            jQuery(text_node).before(`<a class="ilc_qimgd_ImageDetailsLink" href="${questions[a_id].path}${img}" target="_blank">`
							+ `<img class="ilc_qimg_QuestionImage" src="${questions[a_id].path}thumb.${img}" onerror="this.removeAttribute('onerror'); this.style.maxHeight='${questions[a_id].thumb}px'; this.style.maxWidth='${questions[a_id].thumb}px'; this.src='${questions[a_id].path}${img}';" /></a>`);
          } else {
...
ilias.questions.assSingleChoice = function (a_id) {
  const a_node = jQuery(`input[name="answers${a_id}"]`);
...
ilias.questions.assKprimChoice = function (a_id) {
  ...
  for (let i = 0; i < questions[a_id].answers.length; i++) {
    const answer = questions[a_id].answers[i];
    const input = jQuery(`input[name="kprim_choice_${a_id}_result_${answer.order}"]:checked`);
...
ilias.questions.assOrderingQuestion = function (a_id) {
  const result = jQuery(`#order${a_id}`).sortable('toArray');
...
ilias.questions.handleOrderingImages = function (a_id) {
  ...
  jQuery(`ul#order${a_id} div.answertext`).each((id, node) => {
    const src = jQuery(node).html();
...
ilias.questions.assMatchingQuestion = function (a_id) {
  (function ($) {
    ...
    let selected = 0; let foundCorrect = 0; let
      foundWrong = 0;

    for (let i = 0; i < questionData.definitions.length; i++) {
      var definition = questionData.definitions[i];
      const dropArea = $(`#definition_${definition.id}`);

      const selectedTerms = dropArea.find('input[type=hidden]');

      selected += selectedTerms.length;

      selectedTerms.each((key, term) => {
        answerData.choice.push(`${definition.id}-${$(term).val()}`);

        let found = false;

        for (let j = 0; j < questionData.matchingPairs.length; j++) {
          const matching = questionData.matchingPairs[j];
...
ilias.questions.assTextSubset = function (a_id) {
  ...
  let wrong_answer_given = false;
  const a_node = jQuery(`input[name="answers${a_id}[]"]`);
  for (let i = 0; i < a_node.length; i++) {
    let answer = a_node.get(i).value;
    ...
    let found = false;
    for (let c = 0; c < questions[a_id].correct_answers.length; c++) {
      let correct_answer = questions[a_id].correct_answers[c].answertext;

      // decode the HTML entity encoding of the correct answer
      const parsed = new DOMParser().parseFromString(correct_answer, 'text/html');
      ...
      if (correct_answer == answer && questions[a_id].correct_answers[c].points > 0) {
        ...
        for (let j = 0; j < i; j++) {
          let old_answer = a_node.get(j).value;
          if (questions[a_id].matching_method == 'ci') {
...
ilias.questions.assClozeTest = function (a_id) {
  ...
  for (let i = 0; i < questions[a_id].gaps.length; i++) {
    const { type } = questions[a_id].gaps[i];
    // select
    if (type == 1) {
      ...
      // numeric
        ...
        for (var j = 0; j < questions[a_id].gaps[i].item.length; j++) {
          ...
          const lb = parseFloat(questions[a_id].gaps[i].item[j].lowerbound);
          const ub = parseFloat(questions[a_id].gaps[i].item[j].upperbound);
...
ilias.questions.initClozeTest = function (a_id) {
  ...
  _initClozeTestCallBack = function (found) {
    const { type } = questions[a_id].gaps[closecounter];
    let input;
    if (type == 0 || type == 2) {
      let { size } = questions[a_id].gaps[closecounter];
    ...
    if (type == 1) {
      ...
      for (let i = 0; i < items.length; i++) {
        const option = jQuery.create('option', { id: i, value: i }, items[i].value);
        input.append(option);
      }
    }
    closecounter++;
  ...
  const parsed = jQuery(`div#${a_id}`).get(0).innerHTML.replace(
    /\[gap\][^\[]+\[\/gap\]/g,
    () => _initClozeTestCallBack(),
  );
  jQuery(`div#${a_id}`).html(parsed);
};

ilias.questions.initLongMenu = function (a_id) {
  const text = $(`#${a_id}`).html().split(/\[Longmenu \d+\]/);
  let replaced_text = '';
  for (const key in text) {
    ...
    if (key < text.length - 1) {
      if (questions[a_id].correct_answers[key][2] == 1) {
        ...
        for (const answer_id in questions[a_id].answers[key]) {
  ...
  $($('.long_menu_input'), $(`#${a_id}`)).each(function (index) {
    const longest = questions[a_id].answers[index].reduce((a, b) => (a.length > b.length ? a : b));
...
ilias.questions.assLongMenu = function (a_id) {
  ...
  for (let i = 0; i < questions[a_id].correct_answers.length; i++) {
...
ilias.questions.selectErrorText = function (a_id, node) {
  ...
  const id = jQuery(node).parent().attr('id');
  const id_index = jQuery.inArray(id, questions[a_id].selected);
...
ilias.questions.assErrorText = function (a_id) {
  ...
  const selected_words = questions[a_id].selected.sort();
...
ilias.questions.showFeedback = function (a_id) {
  ...
  } else if (questions[a_id].nr_of_tries != 0) {
    jQuery(`#feedback${a_id}`).addClass('ilc_qfeedw_FeedbackWrong');

    const rem = questions[a_id].nr_of_tries - answers[a_id].tries;
...
ilias.questions.updateSuccessStatus = function () {
  let s_key;
  let status = ilias.questions.determineSuccessStatus();
...
ilias.questions.determineSuccessStatus = function () {
  ...
  for (const k in questions) {
    ...
    if (!isNaN(index)) {
      ...
      if (!answers[index]) {
        status = 'failed';
      } else if (answers[index].passed != true) { status = 'failed'; }
    }
  }
  return status;
};
...
ilias.questions.showCorrectAnswers = function (a_id) {
  switch (questions[a_id].type) {
    ...
    case 'assKprimChoice':
      for (var i = 0; i < questions[a_id].answers.length; i++) {
        const correctness = questions[a_id].answers[i].correctness ? 1 : 0;

        const inputs = jQuery(`input[name="kprim_choice_${a_id}_result_${questions[a_id].answers[i].order}"]`);
    ...
    case 'assMatchingQuestion':
      (function ($) {
        ...
        const matchings = questions[a_id].matchingPairs;
        const { engineInstance } = questions[a_id];

        engineInstance.reset();

        $(matchings).each((pos, matching) => {
    ...
    case 'assTextSubset':
      ...
      if (correct_info.length) {
        const elements = jQuery(`#container${a_id} > .ilc_answers`);
        if (correct_count > 1) {
    ...
    case 'assErrorText':
      ...
      for (let i = 0; i < correct_answers.length; i++) {
        const node = document.getElementById(correct_answers[i].pos);
        if (node.length === 0) {
          continue;
        }
...
ilias.questions.fix_imageurls = function (node) {
  const container = jQuery(node).find('img');
  ...
  switch (typeof v) {
    ...
    case 'object':
      if (v === null) {
        ...
        const ra = new Array();
        for (let i = 0, ni = v.length; i < ni; i += 1) {
...
jQuery.fn.textLimiter = function () {
  return this.each(function () {
    ...
    const counter_id	 = `counter${nr}`;
    const max			 = this.getAttribute('maxlength');
    const html_counter = `<br>Remaining characters: <span id="${counter_id}" class="counter"><span>${max}</span></div>`;
    jQuery(this).after(html_counter);
    const jquery_pattern = `#${counter_id} > span`;
    ...
    jQuery(this).bind('keyup', function () {
      ...
      const left_over = maxLength - currentLength;
...
(function (jQuery) {
  ...
  jQuery.extend({
    create(element, attributes, children) {
      // create new element
      const elem = jQuery(document.createElement(element));
...
(function (jQuery) {
  let has_VML; let create_canvas_for; let add_shape_to; let clear_canvas; let shape_from_area;
  let canvas_style; let fader; let hex_to_decimal; let css3color; let
    is_image_loaded;
  ...
  create_canvas_for = function (img, id) {
    let width = jQuery(img).prop('width');
    let height = jQuery(img).prop('height');
    if (typeof (img.width) === 'number') {
      width = img.width;
      height = img.height;
    }
    const c = jQuery(`<canvas id="canvas_${id}" style="width:${width}px;height:${height}px;"></canvas>`).get(0);
    c.width = width;
    c.height = height;
  ...
  jQuery.fn.maphilight_mod = function (opts) {
    ...
    return this.each(function () {
      let img; let wrap; let options; let map; let canvas; let canvas_always; let mouseover; let highlighted_shape; let
        question_id;
      img = jQuery(this);
      ...
      mouseover = function (e) {
        ...
        if (area_options.linked) {
          const thislinked = area_options.linked;

          jQuery(map).find('area[coords]').each(function () {
            let shape; let area_options; let
              object;
            area_options = jQuery.metadata ? jQuery.extend({}, options, jQuery(this).metadata()) : options;
      ...
      draw = function (object, target_canvas) {
        let shape; let area_options; var
          object;
        area_options = jQuery.metadata ? jQuery.extend({}, options, jQuery(this).metadata()) : options;

        const arr_map = jQuery(object).attr('id').split('_');
        const str_cmap = `.cmap${arr_map[0]}`;
      ...
      if (options.alwaysOn) {
        ...
        jQuery(map).find('area[coords]').mouseover(mouseover).mouseout((e) => { clear_canvas(canvas); });
      ...
      if (ilias.questions.answers[question_id] && ilias.questions.answers[question_id].passed != true) {
        for (i = 0; i < ilias.questions.answers[question_id].areas.length; i++) {
          if (ilias.questions.answers[question_id].areas[i] == true) {
            const canvas_id = `${question_id}_${i}`;
            if (!jQuery(`#canvas_${canvas_id}`).attr('id')) {
              const selected_area = jQuery(map).find(`area[id="${canvas_id}"]`);
