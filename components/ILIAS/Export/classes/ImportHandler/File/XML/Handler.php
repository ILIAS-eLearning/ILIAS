...
use ILIAS\Export\ImportHandler\I\File\Namespace\HandlerInterface as ilImportHandlerFileNamespaceInterface;
...
use ILIAS\Export\ImportStatus\StatusType as ilImportHandlerStatusType;
use SplFileInfo;

class Handler extends ilImportHandlerFile implements ilImportHandlerXMLFileInterface
{
    ...
    ): ilImportHandlerXMLFileInterface {
        $clone = clone $this;
    ...
    public function withAdditionalNamespace(
        ilImportHandlerFileNamespaceInterface $namespace_handler
    ): ilImportHandlerXMLFileInterface {
        $clone = clone $this;
    ...
    public function loadDomDocument(): DOMDocument
    {
        ...
        foreach ($errors as $error) {
            $status_collection = $status_collection->withAddedStatus(
                $this->status->handler()->withType(ilImportHandlerStatusType::FAILED)->withContent(
        ...
        if ($status_collection->hasStatusType(ilImportHandlerStatusType::FAILED)) {
            $exception = $this->status->exception($status_collection->toString(ilImportHandlerStatusType::FAILED));
