...
class ilCategoryWizardInputGUI extends ilTextInputGUI
{
    ...
    public function insert(
        ...
        $tpl = new ilTemplate("tpl.prop_categorywizardinput.html", true, true, "components/ILIAS/SurveyQuestionPool_");
        if (is_object($this->values)) {
            for ($i = 0; $i < $this->values->getCategoryCount(); $i++) {
                $cat = $this->values->getCategory($i);
                if (!$cat->neutral) {
        ...
        $tpl = $this->tpl;
        $tpl->addJavaScript("./components/ILIAS/Form_/js/ServiceFormWizardInput.js");
        $tpl->addJavaScript("./components/ILIAS/SurveyQuestionPool_/Categories/js/categorywizard.js");
    }
}
