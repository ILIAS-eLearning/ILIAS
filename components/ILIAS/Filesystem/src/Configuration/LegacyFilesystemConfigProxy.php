 ...
 * @internal Delegated to global DIC
 */
class LegacyFilesystemConfigProxy implements FilesystemConfig
{
    ...
    protected function fromSettingsTable(string $module, string $key, mixed $default = null): mixed
    {
        ...
        try {
            ...
            return $this->resolved_values[$module][$key] = ($DIC->database()->fetchAssoc($res)['value'] ?? $default);
        } catch (\Throwable) {
            throw new \LogicException('Cannot read configuration during bootstrap');
        }
    }
    ...
    private function readWhiteList(): void
    {
        ...
        $this->white_list_negative = array_map(
            $cleaner,
            explode(",", (string) $this->fromSettingsTable(self::MODULE_NAME, "suffix_repl_additional", ''))
        );

        $this->white_list_positive = array_map(
            $cleaner,
            explode(",", (string) $this->fromSettingsTable(self::MODULE_NAME, "suffix_custom_white_list", ''))
        );
    ...
    private function readBlackList(): void
    {
        ...
        $this->black_list_prohibited = array_map(
            $cleaner,
            explode(",", (string) $this->fromSettingsTable(self::MODULE_NAME, "suffix_custom_expl_black", ''))
        );
