il.Timeline = {
  compressEntries() {
    let minspace; let prev; let el_top; let prev_badge_top; let el; let mt; let mt2; let mt3; let prev_top; let
      d;
    const t = il.Timeline;
      ...
      .each(function () {
        ...
        if (prev.length) {
          ...
          if ($(el).prevAll('li').find('.ilTimelineBadge').length > 0) {
            prev_badge_top = $(el).prevAll('li').find('.ilTimelineBadge').first()
              .position().top;
            if (prev_badge_top > 0) {
