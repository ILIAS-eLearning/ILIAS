...
4. If cmi5 is used, then ILIAS generates and sends only the following fixed set of statements on learning activities to the LRS:
...
- The settings offer an option to reduce the amount of statements being send from the LRP / content to LRS.
...
Therefore, you MUST ensure that the data produced is properly pseudonymized, as outlined in the section on storage. 
