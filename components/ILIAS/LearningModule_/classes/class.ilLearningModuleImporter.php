...
class ilLearningModuleImporter extends ilXmlImporter
{
    ...
    public function init(): void
    {
        ...
        $this->config = $this->getImport()->getConfig("components/ILIAS/LearningModule_");
    ...
    {
        $pg_map = $a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "pg");
        ...
        // header footer page
        foreach ($a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "lm_header_page") as $old_id => $dummy) {
            $new_page_id = (int) $a_mapping->getMapping("components/ILIAS/LearningModule_", "pg", $old_id);
            if ($new_page_id > 0) {
                $lm_id = ilLMPageObject::_lookupContObjID($new_page_id);
                ilObjLearningModule::writeHeaderPage($lm_id, $new_page_id);
            }
        }
        foreach ($a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "lm_footer_page") as $old_id => $dummy) {
            $new_page_id = (int) $a_mapping->getMapping("components/ILIAS/LearningModule_", "pg", $old_id);
            if ($new_page_id > 0) {
                $lm_id = ilLMPageObject::_lookupContObjID($new_page_id);
        ...
        $link_map = $a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "link");
        ...
        foreach ($pages as $p) {
            ...
            if (count($id) == 3) {
                if (ilPageObject::_exists($id[0], $id[1], $id[2], true)) {
                    ...
                    //$a_mapping->addMapping("components/ILIAS/LearningModule_", "link",
                    if ($this->config->getTranslationImportMode()) {
        ...
        // assign style
        /*
        $alls_map = $a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "lm_style");
        foreach ($alls_map as $new_lm_id => $old_style_id) {
            $new_style_id = (int) $a_mapping->getMapping("components/ILIAS/Style_", "sty", $old_style_id);
        ...
        $lm_map = $a_mapping->getMappingsOfEntity("components/ILIAS/LearningModule_", "lm");
