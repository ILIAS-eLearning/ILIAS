...
class ilDataCollectionDataSet extends ilDataSet
{
    ...
    public function importRecord(
        ...
        switch ($a_entity) {
            case 'dcl':
                if ($new_id = $a_mapping->getMapping('components/ILIAS/Container_', 'objs', $a_rec['id'])) {
                ...
                $a_mapping->addMapping('components/ILIAS/DataCollection_', 'dcl', $a_rec['id'], (string)$new_obj->getId());
                break;
            case 'il_dcl_table':
                ...
                $a_mapping->addMapping('components/ILIAS/DataCollection_', 'il_dcl_table', $a_rec['id'], (string)$table->getId());
                break;
            case 'il_dcl_tableview':
                $new_table_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_table', $a_rec['table_id']);
                if ($new_table_id) {
                    ...
                    $a_mapping->addMapping(
                        'components/ILIAS/DataCollection_',
                        'il_dcl_tableview',
                        $a_rec['id'],
                        (string) $tableview->getId()
                    );
                    $a_mapping->addMapping('components/ILIAS/COPage_', 'pg', 'dclf:' . $a_rec['id'], 'dclf:' . $tableview->getId());
                }
                break;
            case 'il_dcl_field':
                $new_table_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_table', $a_rec['table_id']);
                if ($new_table_id) {
                    ...
                    $field->doCreate();
                    $a_mapping->addMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $a_rec['id'], $field->getId());
            ...
            case 'il_dcl_tfield_set':
                $new_table_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_table', $a_rec['table_id']);
                $new_field_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $a_rec['field']);
                if ($new_table_id) {
            ...
            case 'il_dcl_view':
                ...
                if ($new_table_id) {
                    ...
                    if ($a_rec['type'] == 0 && $a_rec['formtype'] == 0) { //set page_object to tableview
                        // This mapping is needed for the import handled by components/ILIAS/COPage_
                        $a_mapping->addMapping(
                            'components/ILIAS/COPage_',
                            'pg',
                            'dclf:' . $a_rec['id'],
            ...
            case 'il_dcl_viewdefinition':
                $map = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_view', $a_rec['view_id']);
                $new_field_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $a_rec['field']);
            ...
            case 'il_dcl_sel_opts':
                $new_field_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $a_rec['field_id']);
                if ($new_field_id) {
            ...
            case 'il_dcl_record_field':
                $record_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_record', $a_rec['record_id']);
                $field_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $a_rec['field_id']);
                    ...
                    if (is_object($record_field)) {
                        ...
                        switch ($record_field->getField()->getDatatypeId()) {
                            case ilDclDatatype::INPUTFORMAT_MOB:
                                ...
                                $new_mob_id = $a_mapping->getMapping('components/ILIAS/MediaObjects_', 'mob', $a_rec['value']);
                                $value = ($new_mob_id) ? (int) $new_mob_id : null;
                                $this->import_temp_new_mob_ids[] = $new_mob_id;
                                break;
                            case ilDclDatatype::INPUTFORMAT_FILEUPLOAD:
                                $new_file_id = $a_mapping->getMapping('components/ILIAS/File_', 'file', $a_rec['value']);
    ...
    public function beforeFinishImport(ilImportMapping $a_mapping): void
    {
        ...
        foreach ($this->import_temp_refs as $record_field_id => $old_record_id) {
            $new_record_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_record', $old_record_id);
            $value = ($new_record_id) ? (int) $new_record_id : null;
            /** @var ilDclBaseRecordFieldModel $record_field */
        ...
        foreach ($this->import_temp_refs_props as $field_prop_id => $prop_value) {
            $new_field_id = $a_mapping->getMapping('components/ILIAS/DataCollection_', 'il_dcl_field', $prop_value);
