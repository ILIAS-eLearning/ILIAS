...
class ilGlossaryImporter extends ilXmlImporter
{
    ...
    public function init(): void
    {
        ...
        $this->config = $this->getImport()->getConfig("components/ILIAS/Glossary_");
    }
    ...
    public function importXmlRepresentation(
        ...
        if ($a_entity == "glo") {
            // case i container
            if ($new_id = $a_mapping->getMapping('components/ILIAS/Container_', 'objs', $a_id)) {
    ...
    public function finalProcessing(
        ...
        $maps = $a_mapping->getMappingsOfEntity("components/ILIAS/Glossary_", "glo");
        foreach ($maps as $old => $new) {
            if ($old != "new_id" && (int) $old > 0) {
                // get all new taxonomys of this object
                $new_tax_ids = $a_mapping->getMapping("components/ILIAS/Taxonomy_", "tax_usage_of_obj", $old);
                ...
                if ($this->exportedFromSameInstallation()) {
                    $advmdco = $a_mapping->getMappingsOfEntity("components/ILIAS/Glossary_", "advmd_col_order");
                    foreach ($advmdco as $id => $order) {
                        ...
                        if ($field_glo_id == $old) {
                            // #17454
                            $new_local_id = $a_mapping->getMapping("components/ILIAS/AdvancedMetaData_", "lfld", $field_id);
                            if ($new_local_id) {
