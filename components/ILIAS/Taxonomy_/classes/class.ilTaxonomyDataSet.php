    ...
    public function importRecord(
        ...
        switch ($a_entity) {
            case "tax":
                ...
                $a_mapping->addMapping("components/ILIAS/Taxonomy_", "tax", $a_rec["Id"], $newObj->getId());
                break;

            case "tax_tree":
                switch ($a_rec["Type"]) {
                    case "taxn":
                        $parent = (int) $a_mapping->getMapping("components/ILIAS/Taxonomy_", "tax_tree", $a_rec["Parent"]);
                        $tax_id = $a_mapping->getMapping("components/ILIAS/Taxonomy_", "tax", $a_rec["TaxId"]);
                        if ($parent == 0) {
            ...
            case "tax_node_assignment":
                ...
                $new_node_id = (int) $a_mapping->getMapping("components/ILIAS/Taxonomy_", "tax_tree", $a_rec["NodeId"] ?? "");
            ...
            case "tax_usage":
                $usage = $a_mapping->getMapping("components/ILIAS/Taxonomy_", "tax_usage_of_obj", $a_rec["ObjId"]);
                if ($usage != "") {
                    $usage .= ":";
                }
                $a_mapping->addMapping(
