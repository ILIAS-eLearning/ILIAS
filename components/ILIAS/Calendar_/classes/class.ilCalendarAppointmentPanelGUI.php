...
class ilCalendarAppointmentPanelGUI
{
    ...
    public function getHTML($a_app): string
    {
        ...
        $this->tpl = new ilTemplate('tpl.appointment_panel.html', true, true, 'components/ILIAS/Calendar_');
            ...
            case ilCalendarCategory::TYPE_OBJ:
                ...
                if ($this->settings->isCGRegistrationEnabled() and $type == 'crs' or $type == 'grp') {
                    if (!$a_app['event']->isAutoGenerated()) {
                        ...
                        $registrations = array();
                        foreach ($reg->getRegisteredUsers(
                            new ilDateTime($a_app['dstart'], IL_CAL_UNIX),
                            new ilDateTime($a_app['dend'], IL_CAL_UNIX)
                        ) as $usr_data) {
                            $usr_id = $usr_data;
                            $this->ctrl->setParameterByClass('ilconsultationhoursgui', 'user', $usr_id);
                            $registrations[] = '<a href="' . $this->ctrl->getLinkTargetByClass(
                        ...
                        if (count($registrations)) {
                            ...
                            $this->tpl->setVariable(
                                'PANEL_TXT_CURRENT_BOOKING',
                                $this->lng->txt('cal_reg_registered_users')
                            );
                ...
                if (!$is_owner) {
                    ...
                    foreach ($entry->getCurrentBookings($a_app['event']->getEntryId()) as $user_id) {
                        if ($link_users) {
                            ...
                            $bookings[] = '<a href="' . $this->ctrl->getLinkTargetByClass(
                                'ilconsultationhoursgui',
                                'showprofile'
                            ) . '">' .
        ...
        if ($cat_info['editable'] and !$a_app['event']->isAutoGenerated()) {
            ...
            $this->tpl->setVariable(
                'PANEL_EDIT_HREF',
                $this->ctrl->getLinkTargetByClass('ilcalendarappointmentgui', 'askEdit')
            );
            ...
            $this->tpl->setVariable(
                'PANEL_DELETE_HREF',
                $this->ctrl->getLinkTargetByClass('ilcalendarappointmentgui', 'askdelete')
            );
            $this->tpl->parseCurrentBlock();
        }
