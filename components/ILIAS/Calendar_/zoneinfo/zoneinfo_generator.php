...
foreach (ilCalendarUtil::_getShortTimeZoneList() as $tz_name => $tmp) {
    ...
    exec(TZ_CONVERT . ' -o components/ILIAS/Calendar_/zoneinfo/' . $name_underscore . '.tmp' . ' ' . $name);

    $reader = fopen('components/ILIAS/Calendar_/zoneinfo/' . $name_underscore . '.tmp', 'r');
    $writer = fopen('components/ILIAS/Calendar_/zoneinfo/' . $name_underscore . '.ics', 'w');
    ...
    fclose($reader);
    fclose($writer);
    unlink('components/ILIAS/Calendar_/zoneinfo/' . $name_underscore . '.tmp');
