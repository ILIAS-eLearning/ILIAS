...
class ByCreationDate extends AbstractBaseSorter implements CollectionSorter
{
    protected function sortResourceIdentification(array $identifications): array
    {
        usort($identifications, function (ResourceIdentification $a, ResourceIdentification $b): int {
            $a_creation_date = $this->resource_builder->get($a)->getCurrentRevision()->getInformation(
            )->getCreationDate();
            $b_creation_date = $this->resource_builder->get($b)->getCurrentRevision()->getInformation(
            )->getCreationDate();
