...
(function($, viewcontrol) {
	viewcontrol.quickfilter = (function($) {
		var onInternalSelect = function(event, signalData, signal, component_id) {
			...
			quickfilter = $('#' + component_id),				//the component itself
			sigdata = {
				'id' : signal,
				'event' : 'sort',
				'triggerer' : quickfilter,
				'options' : {
					'quickfilter': param
				}
			}
			dd = quickfilter.find('.dropdown-toggle');		//the dropdown

			//close dropdown and set current value
			dd.dropdown('toggle');
			dd.contents()[0].data = signalData.triggerer.contents()[0].data  + ' ';

			quickfilter.trigger(signal, sigdata);
		};
