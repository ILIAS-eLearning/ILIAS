...
(function ($, maincontrols) {
    maincontrols.mainbar = (function ($) {
        var mappings = {}, addToolEntry = function (position_id, removeable, hidden) {
            if (removeable === void 0) { removeable = true; }
            if (hidden === void 0) { hidden = false; }
            this.model.actions.addTool(position_id, removeable, hidden);
        }, addPartIdAndEntry = function (position_id, part, html_id, is_tool) {
            if (is_tool === void 0) { is_tool = false; }
            ...
            $(document).on(signal, function (event, signalData) {
                var id = signalData.options.entry_id, action = signalData.options.action, 
                // @ts-ignore
            ...
            // @ts-ignore
            var mb = il.UI.maincontrols.mainbar, amount = mb.renderer.calcAmountOfButtons();
            mb.model.actions.initMoreButton(amount);
            mb.renderer.render(mb.model.getState());
        }, init = function () {
            // @ts-ignore
            var mb = il.UI.maincontrols.mainbar, cookie_state = mb.persistence.read();
...
(function ($, mainbar) {
    mainbar.model = (function ($) {
        ...
        var state, classes = {
            ...
            entry: function (id) { return factories.cloned(classes.entry, { id: id }); },
            // @ts-ignore
            cloned: function (state, params) { return Object.assign({}, state, params); },
            state: function (nu_state) {
                var tmp_state = factories.cloned(state, nu_state);
                for (var idx in tmp_state.entries) {
                    tmp_state.entries[idx].isTopLevel = classes.entry.isTopLevel;
                }
                for (var idx in tmp_state.tools) {
            ...
            entry: {
                engage: function (entry) {
                    entry.engaged = true;
                    entry.hidden = false;
                    return entry;
                },
                disengage: function (entry) {
                    entry.engaged = false;
                    return entry;
                },
                mb_show: function (entry) {
                    entry.hidden = false;
                    return entry;
                },
                mb_hide: function (entry) {
                    entry.hidden = true;
                    entry.engaged = false;
                    return entry;
                }
            ...
            bar: {
                engageTools: function (bar) {
                    bar.tools_engaged = true;
                    return bar;
                },
                disengageTools: function (bar) {
                    bar.tools_engaged = false;
                    return bar;
                },
                anySlates: function (bar) {
                    bar.any_entry_engaged = true;
                    return bar;
                },
                noSlates: function (bar) {
                    bar.any_entry_engaged = false;
                    return bar;
                },
                withMoreButton: function (bar) {
                    bar.more_available = true;
                    return bar;
                },
                withoutMoreButton: function (bar) {
            ...
            initMoreButton: function (max_buttons) {
                ...
                var entry_ids = Object.keys(state.entries), last_entry_id = entry_ids[entry_ids.length - 1], more = state.entries[last_entry_id];
                if (state.any_tools_visible()) {
                    max_buttons--;
                }
                ;
                //get length of top-level entries (w/o) more-button
                var amount_toplevel = helpers.getTopLevelEntries().length - 1;
            ...
            actions: actions,
            getState: function () { return factories.cloned(state, {}); },
...
(function ($, mainbar) {
    mainbar.persistence = (function ($) {
        var cs, storage = function () {
            if (cs) {
                return cs;
            }
            var cookie_name = hash(entry_ids());
            // @ts-ignore
            ...
            var entries = model_state().entries, base = '';
            for (var idx in entries) {
                base = base + idx;
            }
            return base;
        }, hash = function (str) {
            var hash = 0, len = str.length, i, chr;
            for (var i_1 = 0; i_1 < len; i_1 = i_1 + 1) {
                chr = str.charCodeAt(i_1);
    ...
    mainbar.renderer = (function ($) {
        ...
        more = {
            calcAmountOfButtons: function () {
                ...
                // @ts-ignore
                horizontal = il.UI.page.isSmallS, btn_height = parts.tools_button.getElement().height(), btn_width = parts.tools_button.getElement().width(), amount_buttons = Math.floor((window_height - parts.mainbar.getOffsetTop()) / btn_height);
                if (horizontal) {
            ...
            renderEntry: function (entry, is_tool) {
                var triggerer = parts.triggerer.withHtmlId(dom_references[entry.id].triggerer), slate = parts.slate.withHtmlId(dom_references[entry.id].slate);
                if (entry.hidden) {
                ...
                if (entry.engaged) {
                    ...
                    slate.engage();
                    if (entry.removeable) {
                        var remover = parts.remover.withHtmlId(dom_references[entry.id].remover);
            ...
            moveToplevelTriggerersToMore: function (model_state) {
                ...
                var entry_ids = Object.keys(model_state.entries), last_entry_id = entry_ids[entry_ids.length - 1], more_entry = model_state.entries[last_entry_id], more_button = parts.triggerer.withHtmlId(dom_references[more_entry.id].triggerer), more_slate = parts.slate.withHtmlId(dom_references[more_entry.id].slate), 
                // @ts-ignore
                root_entries = il.UI.maincontrols.mainbar.model.getTopLevelEntries(), root_entries_length = root_entries.length - 1, max_buttons = more.calcAmountOfButtons() - 1; //room for the more-button
                if (model_state.any_tools_visible()) {
                    max_buttons--;
                }
                ;
                for (var i = max_buttons; i < root_entries_length; i++) {
                    var btn = parts.triggerer.withHtmlId(dom_references[root_entries[i].id].triggerer);
            ...
            render: function (model_state) {
                ...
                for (var idx in model_state.entries) {
                    actions.renderEntry(model_state.entries[idx], false);
                }
                for (var idx in model_state.tools) {
