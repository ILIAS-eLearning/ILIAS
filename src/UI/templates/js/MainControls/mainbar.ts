        ...
        var
            ...
            factories = {
                ...
                state: function (nu_state) {
                    ...
                    for (let idx in tmp_state.entries) {
                        tmp_state.entries[idx].isTopLevel = classes.entry.isTopLevel;
                    }
                    for (let idx in tmp_state.tools) {
            ...
            actions = {
                ...
                initMoreButton: function (max_buttons) {
                    ...
                    let amount_toplevel = helpers.getTopLevelEntries().length - 1;
    ...
    mainbar.persistence = (function ($) {
        var
            ...
            storage = function () {
                ...
                let cookie_name = hash(entry_ids());
                // @ts-ignore
            ...
            actions = {
                ...
                renderEntry: function (entry, is_tool) {
                    ...
                    if (entry.engaged) {
                        ...
                        slate.engage();
                        if (entry.removeable) {
                            let remover = parts.remover.withHtmlId(dom_references[entry.id].remover);
                ...
                moveToplevelTriggerersToMore: function (model_state) {
                    ...
                    for (let i = max_buttons; i < root_entries_length; i++) {
                        let btn = parts.triggerer.withHtmlId(dom_references[root_entries[i].id].triggerer);
                ...
                render: function (model_state) {
                    ...
                    for (let idx in model_state.entries) {
                        actions.renderEntry(model_state.entries[idx], false);
                    }
                    for (let idx in model_state.tools) {
