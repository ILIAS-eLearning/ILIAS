...
class ilPaymentShoppingCartGUI extends ilPaymentBaseGUI
{
	...
	*/
	...
	{		
		...
		ilUtil::sendInfo($this->lng->txt('pay_bill_thanks'), true);
	...
	function checkCouponsOfShoppingCart()
	{
...
#							$buttonParams["amount_".($counter+1)] = $price_arr['unit_value'].".".$price_arr['sub_unit_value'];
					...
					$tpl->setVariable("COUPON_TABLE", $this->__showCouponInput($pay_methods[$p]));
...
#							}
	...
	function __showCouponInput($payment_type = '')
	{
		include_once './payment/classes/class.ilGeneralSettings.php';
		$genSet = new ilGeneralSettings();
