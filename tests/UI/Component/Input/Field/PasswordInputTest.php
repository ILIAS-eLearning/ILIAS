...
class PasswordInputTest extends ILIAS_UI_TestBase {
	...
	public function test_render() {
		...
		$expected = "<div class=\"form-group row\">" . "	<label for=\"$name\" class=\"control-label col-sm-3\">$label</label>"
		            . "	<div class=\"col-sm-9\">" . "		<input type=\"password\" name=\"$name\" class=\"form-control form-control-sm\" />"
	...
	public function test_render_required() {
		...
		$expected = "<div class=\"form-group row\">" . "	<label for=\"$name\" class=\"control-label col-sm-3\">" . "$label"
		            ...
		            . "		<input type=\"password\" name=\"$name\" class=\"form-control form-control-sm\" />" . "		" . "		" . "	</div>"
		            . "</div>";
		$this->assertEquals($expected, $html);
	}
	...
	public function test_value_not_changeable() {
		$f = $this->buildFactory();
		$label = "label";
		$pwd = $f->password($label);
		$this->assertNull($pwd->getValue());

		$pwd = $pwd->withValue('some value');
		$this->assertNull($pwd->getValue());
