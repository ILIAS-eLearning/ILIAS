...
class ToastTest extends ILIAS_UI_TestBase
{
    ...
    /**
     * @dataProvider toast_container_provider
     */
    public function test_toast_container(int $vanish_time, int $delay_time): void
    {
        $container = $this->getToastFactory()->container()
            ->withAdditionalToast($this->getToastFactory()->standard('', $this->getIconFactory()->standard('', '')))
    ...
    public function toast_container_provider(): array
    {
        return [
            [5000, 500, ],
            [-5000, -500],
