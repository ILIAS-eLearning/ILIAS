...
(function () {
    ...
    YAHOO.extend(Carousel, YAHOO.util.Element, {
        ...
        scrollTo: function (item, dontSelect) {
            var carousel   = this,
                animate, animCfg, isCircular, isVertical, rows, delta, direction, firstItem, lastItem, firstVisible, itemsPerCol,
                itemsPerRow, numItems, numPerPage, offset, page, pageForItem, rv, sentinel,
                stopAutoScroll;
        ...
        _pagerClickHandler: function (ev) {
            ...
            function getPagerNode(el) {
                ...
                if (elNode == itemEl.toUpperCase()) {
                    ...
                    el = el.parentNode;// item is the em and not the anchor (when text is visible)          
                }
            ...
            if(rows && cols) {
                ...
                for (var i = 0; i < num; i++) {
                    ...
                    if(item) {                        
                        ...
                        if (i % cols === 0) {                            
                            delta = Math.floor(i/carousel.get("numVisible"));// find page item belongs on. 
                            col = delta * cols;// shifts cols to appropriate page.  
                            row++;// breaks a row.
                        } 
                
                        if (i % carousel.get("numVisible") === 0) {
        ...
        _setContainerSize: function (clip, attr) {
            ...
            if (isVertical) {
                ...
                Dom.setStyle(carousel._carouselEl + (0), "width", size + "px");// Bug fix for vertical carousel (goes in conjunction with .yui-carousel-element {... 3200px removed from styles), and allows for multirows in IEs). 
                ...
                carousel.setStyle("width",  size + (0) + "px");    
            } else {
                if(rows) {
