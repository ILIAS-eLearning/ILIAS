--TEST--
PEAR_Validate->validate(), stability tests
--SKIPIF--
<?php
...
$phpunit->assertEquals(array (
  ...
  array (
    ...
    array (
      'field' => 'state',
      'reason' => 'invalid release stability "frong", must be one of: snapshot, devel, alpha, beta, stable',
    ),
  ),
), $val->getFailures(), 'failures frong');

$pf->setAPIStability('frong');
$res = $val->validate(PEAR_VALIDATE_NORMAL);
$phpunit->assertFalse($res, 'frong');
$phpunit->showall();
$phpunit->assertEquals(array (
  ...
  array (
    ...
    array (
      'field' => 'state',
      'reason' => 'invalid API stability "frong", must be one of: devel, alpha, beta, stable',
    ),
  ),
), $val->getFailures(), 'failures frong');

$pf->setAPIStability('snapshot');
$res = $val->validate(PEAR_VALIDATE_NORMAL);
$phpunit->assertFalse($res, 'API snapshot');
$phpunit->showall();
$phpunit->assertEquals(array (
  'warnings' => 
  array (
  ...
  array (
    ...
    array (
      'field' => 'state',
      'reason' => 'invalid API stability "snapshot", must be one of: devel, alpha, beta, stable',
    ),
  ),
), $val->getFailures(), 'failures API snapshot');

$pf->setAPIStability('stable');
$pf->setReleaseStability('snapshot');
$res = $val->validate(PEAR_VALIDATE_NORMAL);
