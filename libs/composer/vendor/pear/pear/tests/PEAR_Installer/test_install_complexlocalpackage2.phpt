--TEST--
PEAR_Installer->install() with complex local package.xml 2.0 [preferred_state = alpha]
--SKIPIF--
<?php
...
$phpunit->assertEquals($result[1]->_downloader->getDownloadDir() . DIRECTORY_SEPARATOR . 'Bar-1.5.2.tgz',
...
$phpunit->assertEquals($result[2]->_downloader->getDownloadDir() . DIRECTORY_SEPARATOR . 'Foobar-1.5.0a1.tgz',
    $dlpackages[2]['file'], 'file 2');
$phpunit->assertIsa('PEAR_PackageFile_v2', $dlpackages[2]['info'], 'info 2');
$phpunit->assertEquals('Foobar',           $dlpackages[2]['pkg'], 'Foobar');
