--TEST--
PEAR_PackageFile_Parser_v2->setVersion()
--SKIPIF--
<?php
...
$pf->setReleaseVersion('1.5.3');
$phpunit->assertEquals('1.5.3', $pf->getVersion(), 'release set failed');
...
$pf->setAPIVersion('1.1.0');
$phpunit->assertEquals('1.1.0', $pf->getVersion('api'), 'api set failed');
$result = $pf->validate(PEAR_VALIDATE_NORMAL);
$phpunit->assertEquals(array(), $fakelog->getLog(), 'normal validate empty log');
$phpunit->assertNoErrors('after validation');
