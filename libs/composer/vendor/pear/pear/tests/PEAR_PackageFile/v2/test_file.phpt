--TEST--
PEAR_PackageFile_Parser_v2 file manipulation (unfinished)
--SKIPIF--
<?php
...
$pf->setURI('http://www.example.com/fakepackage.tgz');
$phpunit->assertEquals('__uri', $pf->getChannel(), 'fake channel failed');
$phpunit->assertEquals('http://www.example.com/fakepackage.tgz', $pf->getURI(), 'set failed');
