...
class Mp3 extends BaseHandler
{
    ...
    public function analyze()
    {
        ...
        if (isset($info['mpeg']['audio']['LAME'])) {
            ...
            if (!empty($info['mpeg']['audio']['LAME']['long_version'])) {
                $info['audio']['encoder'] = rtrim($info['mpeg']['audio']['LAME']['long_version'],
                                                  "\x00");
            } elseif (!empty($info['mpeg']['audio']['LAME']['short_version'])) {
                $info['audio']['encoder'] = rtrim($info['mpeg']['audio']['LAME']['short_version'],
                                                  "\x00");
            }
        }
        ...
        if (!empty($CurrentDataLAMEversionString) && (substr($CurrentDataLAMEversionString,
                                                             0, 6) == 'LAME3.') && !preg_match('[0-9\)]',
                                                                                               substr($CurrentDataLAMEversionString,
                                                                                                      -1))) {
            ...
            $PossiblyLongerLAMEversion_Data = fread($this->getid3->fp,
            ...
            switch (substr($CurrentDataLAMEversionString, -1)) {
                ...
                case 'b':
                    ...
                    $CurrentDataLAMEversionString = substr($CurrentDataLAMEversionString,
                                                           0, -1);
                    break;
            }
            if (($PossiblyLongerLAMEversion_String = strstr($PossiblyLongerLAMEversion_Data,
                                                            $CurrentDataLAMEversionString)) !== false) {
                if (substr($PossiblyLongerLAMEversion_String, 0,
                           strlen($CurrentDataLAMEversionString)) == $CurrentDataLAMEversionString) {
                    $PossiblyLongerLAMEversion_NewString = substr($PossiblyLongerLAMEversion_String,
                                                                  0,
                                                                  strspn($PossiblyLongerLAMEversion_String,
                                                                         'LAME0123456789., (abcdefghijklmnopqrstuvwxyzJFSOND)')); //"LAME3.90.3"  "LAME3.87 (beta 1, Sep 27 2000)" "LAME3.88 (beta)"
    ...
    /**
     *
     * @staticvar array $NamedPresetBitrates
     * @staticvar array $KnownEncoderValues
     * @staticvar array $ExpectedLowpass
     * @staticvar array $ExpectedResampledRate
     * @return type
     */
    public function GuessEncoderOptions()
    {
        ...
        } elseif (!empty($thisfile_mpeg_audio_lame['preset_used']) && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'],
                                                                                 $NamedPresetBitrates))) {
     ...
     * @staticvar type $MPEGaudioFrequencyLookup
     * @staticvar type $MPEGaudioChannelModeLookup
     * @staticvar type $MPEGaudioModeExtensionLookup
     * @staticvar type $MPEGaudioEmphasisLookup
     * @staticvar array $MPEGaudioHeaderDecodeCache
     * @staticvar array $MPEGaudioHeaderValidCache
     * @param  type    $offset
     * @param  type    $info
     * @param  type    $recursivesearch
     * @param  type    $ScanAsCBR
     * @param  type    $FastMPEGheaderScan
     * @return boolean
     */
    public function decodeMPEGaudioHeader($offset, &$info,
                                          ...
                                          $FastMPEGheaderScan = false)
    {
        ...
        if (!isset($MPEGaudioHeaderValidCache[$head4])) { // Not in cache
            ...
            $MPEGaudioHeaderValidCache[$head4] = self::MPEGaudioHeaderValid($MPEGheaderRawArray,
                                                                            false,
                                                                            false);
        }
        ...
        if ($info['audio']['sample_rate'] > 0) {
            $thisfile_mpeg_audio['framelength'] = self::MPEGaudioFrameLength($thisfile_mpeg_audio['bitrate'],
        ...
        if (substr($headerstring, 4 + 32, 4) == 'VBRI') {
            ...
            $VBRidOffset = self::XingVBRidOffset($thisfile_mpeg_audio['version'],
                                                 $thisfile_mpeg_audio['channelmode']);
            $SideInfoData = substr($headerstring, 4 + 2, $VBRidOffset - 4);

            if ((substr($headerstring, $VBRidOffset, strlen('Xing')) == 'Xing') || (substr($headerstring,
                                                                                           $VBRidOffset,
                                                                                           strlen('Info')) == 'Info')) {
...
//				}
                ...
                if (substr($headerstring, $VBRidOffset + 120, 4) == 'LAME') {
                    ...
                    $thisfile_mpeg_audio_lame['long_version'] = substr($headerstring,
                                                                       $VBRidOffset + 120,
                                                                       20);
                    $thisfile_mpeg_audio_lame['short_version'] = substr($thisfile_mpeg_audio_lame['long_version'],
                                                                        0, 9);
                    ...
                    if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.90') {
                        ...
                        $thisfile_mpeg_audio_lame['short_version'] = substr($headerstring,
                ...
                if ($recursivesearch) {
                    $thisfile_mpeg_audio['bitrate_mode'] = 'vbr';
                    if ($this->RecursiveFrameScanning($offset,
                                                      $nextframetestoffset, true)) {
                        $recursivesearch = false;
                        $thisfile_mpeg_audio['bitrate_mode'] = 'cbr';
                    }
        ...
        if ($recursivesearch) {
            ...
            if (!$this->RecursiveFrameScanning($offset, $nextframetestoffset,
                                               $ScanAsCBR)) {
                return false;
            }
        }
     ...
     *
     * @param  type    $offset
     * @param  type    $nextframetestoffset
     * @param  type    $ScanAsCBR
     * @return boolean
     */
    public function RecursiveFrameScanning(&$offset, &$nextframetestoffset,
                                           $ScanAsCBR)
    {
        $info = &$this->getid3->info;
        ...
        for ($i = 0; $i < self::GETID3_MP3_VALID_CHECK_FRAMES; $i++) {
            // check next self::GETID3_MP3_VALID_CHECK_FRAMES frames for validity, to make sure we haven't run across a false synch
            ...
            if ($this->decodeMPEGaudioHeader($nextframetestoffset,
    ...
    public function FreeFormatFrameLength($offset, $deepscan = false)
    {
        ...
        if (!$framelength) {
            ...
            $framelength1 = strpos($MPEGaudioData, substr($SyncPattern1, 0, 3),
                                                          4);
            $framelength2 = strpos($MPEGaudioData, substr($SyncPattern2, 0, 3),
                                                          4);
        ...
        if ($deepscan) {
            ...
            while ($nextoffset < ($info['avdataend'] - 6)) {
                ...
                if ((substr($NextSyncPattern, 1, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                                                                                                     1,
                                                                                                     strlen($SyncPattern2)) == $SyncPattern2)) {
                    // good - found where expected
                    $ActualFrameLengthValues[] = $framelength;
                } elseif ((substr($NextSyncPattern, 0, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                                                                                                           0,
                                                                                                           strlen($SyncPattern2)) == $SyncPattern2)) {
                    // ok - found one byte earlier than expected (last frame wasn't padded, first frame was)
                    $ActualFrameLengthValues[] = ($framelength - 1);
                    $nextoffset--;
                } elseif ((substr($NextSyncPattern, 2, strlen($SyncPattern1)) == $SyncPattern1) || (substr($NextSyncPattern,
                                                                                                           2,
                                                                                                           strlen($SyncPattern2)) == $SyncPattern2)) {
    ...
    public function getOnlyMPEGaudioInfoBruteForce()
    {
        ...
        while (ftell($this->getid3->fp) < $info['avdataend']) {
            ...
            if (!isset($MPEGaudioHeaderValidCache[$head4])) {
                $MPEGaudioHeaderValidCache[$head4] = self::MPEGaudioHeaderValid($MPEGaudioHeaderDecodeCache[$head4],
                                                                                false,
                                                                                false);
            }
            if ($MPEGaudioHeaderValidCache[$head4]) {
                ...
                if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                    ...
                    fseek($this->getid3->fp,
                          $MPEGaudioHeaderLengthCache[$head4] - 4, SEEK_CUR);
                    ...
                    if ($next4{0} == "\xFF") {
                        ...
                        if (!isset($MPEGaudioHeaderValidCache[$next4])) {
                            $MPEGaudioHeaderValidCache[$next4] = self::MPEGaudioHeaderValid($MPEGaudioHeaderDecodeCache[$next4],
                                                                                            false,
                                                                                            false);
                        }
                        if ($MPEGaudioHeaderValidCache[$next4]) {
                            ...
                            if ($max_frames_scan && (++$frames_scanned >= $max_frames_scan)) {
                                ...
                                $info['warning'][] = 'too many MPEG audio frames to scan, only scanned first ' . $max_frames_scan . ' frames (' . number_format($pct_data_scanned * 100,
                                                                                                                                                                1) . '% of file) and extrapolated distribution, playtime and bitrate may be incorrect.';
    ...
    public function getOnlyMPEGaudioInfo($avdataoffset, $BitrateHistogram = false)
    {
        ...
        $sync_seek_buffer_size = min(128 * 1024,
        ...
        while ($SynchSeekOffset < $sync_seek_buffer_size) {
            ...
            if (($header{$SynchSeekOffset} == "\xFF") && ($header{($SynchSeekOffset + 1)} > "\xE0")) { // synch detected
                if (!isset($FirstFrameThisfileInfo) && !isset($info['mpeg']['audio'])) {
                    ...
                    if (!$this->decodeMPEGaudioHeader($FirstFrameAVDataOffset,
                ...
                if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset,
                                                 $dummy, true)) {
                    $info = $dummy;
                    ...
                    if ($BitrateHistogram) {
                        ...
                        for ($current_segment = 0;
                            ...
                            $scan_start_offset[$current_segment] = max(ftell($this->getid3->fp),
                                                                             $info['avdataoffset'] + round($current_segment * (($info['avdataend'] - $info['avdataoffset']) / $max_scan_segments)));
                            if ($current_segment > 0) {
                                ...
                                for ($j = 0; $j < (strlen($buffer_4k) - 4); $j++) {
                                    if (($buffer_4k{$j} == "\xFF") && ($buffer_4k{($j + 1)} > "\xE0")) { // synch detected
                                        if ($this->decodeMPEGaudioHeader($scan_start_offset[$current_segment] + $j,
                                                                         $dummy,
                                                                         false,
                                                                         false,
                                                                         $FastMode)) {
                                            $calculated_next_offset = $scan_start_offset[$current_segment] + $j + $dummy['mpeg']['audio']['framelength'];
                                            if ($this->decodeMPEGaudioHeader($calculated_next_offset,
                                                                             $dummy,
                                                                             false,
                                                                             false,
                                                                             $FastMode)) {
                            ...
                            while ($this->decodeMPEGaudioHeader($synchstartoffset,
                                                                $dummy, false,
                                                                false, $FastMode)) {
                                $FastMode = true;
                        ...
                        if ($pct_data_scanned > 0) {
                            $info['warning'][] = 'too many MPEG audio frames to scan, only scanned ' . $frames_scanned . ' frames in ' . $max_scan_segments . ' segments (' . number_format($pct_data_scanned * 100,
                                                                                                                                                                                            1) . '% of file) and extrapolated distribution, playtime and bitrate may be incorrect.';
     ...
     *
     * @param  type $head4
     * @param  type $allowBitrate15
     * @return type
     */
    public static function MPEGaudioHeaderBytesValid($head4,
                                                     $allowBitrate15 = false)
    {
        return self::MPEGaudioHeaderValid(self::MPEGaudioHeaderDecode($head4),
                                                                      false,
                                                                      $allowBitrate15);
    }
     ...
     * @return type
     */
    public static function MPEGaudioFrameLength(&$bitrate, &$version, &$layer,
                                                $padding, &$samplerate)
    {
    ...
    /**
     *
     * @staticvar array $standard_bit_rates
     * @staticvar array $bit_rate_table
     * @param  type $bit_rate
     * @return int
     */
    public static function ClosestStandardMP3Bitrate($bit_rate)
    {
        ...
        if (!isset($bit_rate_table[$round_bit_rate])) {
            if ($round_bit_rate > max($standard_bit_rates)) {
                $bit_rate_table[$round_bit_rate] = round($bit_rate,
                                                         2 - strlen($bit_rate));
            } else {
     ...
     * @param  type $SourceSampleFrequencyID
     * @return type
     */
