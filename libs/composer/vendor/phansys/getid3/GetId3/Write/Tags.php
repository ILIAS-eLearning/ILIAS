 ...
 * @uses GetId3\Write\Metaflac (optional)
 * @uses GetId3\Write\Lyrics3 (optional)
 */
class Tags
{
    // public
                ...
                default:
                    foreach ($valuearray as $key => $value) {
                        if (is_string($value) || is_numeric($value)) {
                            $ape_tag_data[$tag_key][$key] = Helper::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
                        } else {
                ...
                if (strstr($value, "\n")) {
                    ...
                    foreach ($multilineexploded as $newcomment) {
                        if (strlen(trim($newcomment)) > 0) {
                            $tag_data_vorbiscomment[$tag_key][] = Helper::iconv_fallback($this->tag_encoding, 'UTF-8', $newcomment);
                        }
                    }
                } elseif (is_string($value) || is_numeric($value)) {
                    $tag_data_vorbiscomment[$tag_key][$key] = Helper::iconv_fallback($this->tag_encoding, 'UTF-8', $value);
                } else {
