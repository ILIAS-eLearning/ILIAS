...
class ilBMFType_duration
{
    ...
    function unix_to_duration($seconds) {
        return ilBMFType_duration::getduration($seconds);
    }
    ...
    function _test($time) {
        if (ilBMFType_duration::is_duration($time)) {
            $t = ilBMFType_duration::duration_to_unix($time);
            echo "Duration: $time is ".$t." seconds\n";
        } else {
            $t = ilBMFType_duration::unix_to_duration($time);
    ...
    function add($d1, $d2) {
        $s1 = ilBMFType_duration::duration_to_unix($d1);
        $s2 = ilBMFType_duration::duration_to_unix($d2);
        return ilBMFType_duration::unix_to_duration($s1 + $s2);
    }
...
ilBMFType_duration::_test($t);
$t = ilBMFType_duration::_test('-P120D');
ilBMFType_duration::_test($t);

// duration since 1970
$t = ilBMFType_duration::_test(time());
ilBMFType_duration::_test($t);

print "Add should be PT0S: ".ilBMFType_duration::add('-P120D','P4M')."\n";
print "Subtract should be PT0S: ".ilBMFType_duration::subtract('P120D','P4M')."\n";
?>