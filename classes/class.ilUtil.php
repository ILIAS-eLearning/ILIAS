...
class ilUtil
{
	...
	*/
	...
	{
		// TO DO:
		// Diese Funktion soll Formfeldeingaben ÅberprÅfen (empty und required)
	}
	...
	*/
	function Linkbar ($AScript,$AHits,$ALimit,$AOffset,$AParams = array())
	{
		...
		// Wenn Hits grˆsser Limit, zeige Links an
		if ($AHits > $ALimit)
		{
			if (!empty($AParams))
			{
			...
			// ¸bergehe "zurÅck"-link, wenn offset 0 ist.
			if ($AOffset >= 1)
			{
			...
			// Benˆtigte Seitenzahl kalkulieren
...
//			}
	...
	*/
	function makeDateSelect($prefix,$date="current")
	{
		...
		$months = array(1 => 'Januar', 'Februar', 'M‰rz', 'April', 'Mai','Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember');
