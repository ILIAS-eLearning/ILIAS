...
<?php
    ...
    while ($row = $ilDB->fetchAssoc($res)) {
        ...
        $ins_res = $ilDB->manipulateF(
            'INSERT INTO payment_currencies (currency_id, unit)
			VALUES(%s,%s)',
            array('integer', 'text'),
            array($nextId, $row['currency_unit'])
        );

        $upd_prices = $ilDB->manipulateF(
            'UPDATE payment_prices SET currency = %s',
            array('integer'),
            array($nextId)
        );
        ...
        while ($rec = $ilDB->fetchAssoc($set)) {
            ...
            if (is_array($old_ex_files[$rec["exc_id"]])) {
                foreach ($old_ex_files[$rec["exc_id"]] as $file) {
                    ...
                    if (is_file($old)) {
                        ilFileUtils::makeDirParents(dirname($new));
                        rename($old, $new);
                        //echo "<br><br>move: ".$old.
//	"<br>to: ".$new;
                    }
...
while ($row = $ilDB->fetchAssoc($res)) {
    if (preg_match("/(\d{4})-(\d{2})-(\d{2})/", $row['startdate'], $matches)) {
        $ilDB->manipulateF(
            'UPDATE svy_svy SET startdate_tmp = %s WHERE survey_id = %s',
            array('text', 'integer'),
...
while ($row = $ilDB->fetchAssoc($res)) {
    if (preg_match("/(\d{4})-(\d{2})-(\d{2})/", $row['enddate'], $matches)) {
        $ilDB->manipulateF(
            'UPDATE svy_svy SET enddate_tmp = %s WHERE survey_id = %s',
            array('text', 'integer'),
            array(sprintf("%04d%02d%02d%02d%02d%02d", $matches[1], $matches[2], $matches[3], 0, 0, 0), $row['survey_id'])
        );
    }
...
$qpl_export_base = ilFileUtils::getDataDir() . "/qpl_data/";
                ...
                while ($q_entry = $q_dir->read()) {
                    if ($q_entry != "." and $q_entry != "..") {
                        if (@is_dir($qpl_export_base . $entry . '/' . $q_entry) && strcmp($q_entry, 'export') == 0) {
                            ...
                            while ($exp_entry = $exp_dir->read()) {
                                if (@is_file($qpl_export_base . $entry . '/' . $q_entry . '/' . $exp_entry)) {
                                    ...
                                    if ($res) {
                                        switch ($matches[4]) {
                                            case 'zip':
                                                if (!@is_dir($qpl_export_base . $entry . '/' . 'export_zip')) {
                                                    ilFileUtils::makeDir($qpl_export_base . $entry . '/' . 'export_zip');
                                                }
                            ...
                            $exp_dir->close();
                            if (@is_dir($qpl_export_base . $entry . '/' . $q_entry)) {
                                ilFileUtils::delDir($qpl_export_base . $entry . '/' . $q_entry);
                            }
                        }
                    }
                }
...
<?php
$set = $ilDB->query(
    "SELECT * FROM sahs_sc13_tree_node WHERE " .
...
<?php
    ...
    while ($row = $ilDB->fetchAssoc($res)) {
        ...
        $access_enddate = date("Y-m-d H:i:s", mktime(
            $orderDateHour,
            $orderDateMinute,
            $orderDateSecond,
            $orderDateMonth + $duration,
            $orderDateDay,
            $orderDateYear
        ));
...
<?php
        ...
        while ($rec = $ilDB->fetchAssoc($sets)) {
            ...
            ilFileUtils::makeDir(CLIENT_WEB_DIR . "/sty");
            ilFileUtils::makeDir(CLIENT_WEB_DIR . "/sty/sty_" . $rec["obj_id"]);
            $tdir = CLIENT_WEB_DIR . "/sty/sty_" . $rec["obj_id"] . "/images";
            ilFileUtils::makeDir($tdir);
...
<?php
  $ins_res = $ilDB->manipulateF(
    'INSERT INTO il_dcl_datatype_prop (id, datatype_id, title, inputformat)
      VALUES(%s,%s,%s,%s)',
...
<?php
$ilDB->manipulateF(
    "INSERT INTO il_dcl_datatype_prop (id,datatype_id,title,inputformat) VALUES " .
           " (%s,%s,%s,%s)",
    array("integer", "integer", "text", "integer"),
...
$bibl_data_dir = ilFileUtils::getDataDir() . "/bibl";
ilFileUtils::makeDir($bibl_data_dir);
?>
<#3848>
<?php
...
//BibTeX
$ilDB->manipulateF(
    'INSERT INTO  il_bibl_overview_model (ovm_id, filetype, literature_type, pattern)
	VALUES(%s, %s, %s, %s)',
    array('integer', 'text', 'text', 'text'),
...
//BibTeX
    $ilDB->manipulateF(
        'UPDATE  il_bibl_overview_model SET filetype = %s, literature_type = %s, pattern = %s WHERE ovm_id = %s',
...
$updateQuestionPoints = $ilDB->prepareManip(
    "UPDATE qpl_questions SET points = ? WHERE question_id = ?",
    array('integer', 'integer')
);
...
$base_path = ilFileUtils::getDataDir();
...
function quotaHandleFile($a_obj_id, $a_owner_id)
{
    ...
    $file_path = ilFileUtils::getDataDir() . "/ilFile/";
    if (count($tpath)) {
        $file_path .= (implode('/', $tpath) . '/');
    }
    $file_path .= "file_" . $a_obj_id;
    if (file_exists($file_path)) {
        $file_size = (int) ilFileUtils::dirsize($file_path);
        if ($file_size > 0) {
...
function quotaHandleVerification($a_type, $a_obj_id, $a_owner_id)
{
    ...
    $file_path = ilFileUtils::getDataDir() . "/ilVerification/";
    if (count($tpath)) {
        $file_path .= (implode('/', $tpath) . '/');
    }
...
if ($ilDB->tableColumnExists('tst_tests', 'random_test')) {
    $ilDB->manipulateF(
        ...
        array('text', 'text'),
        array('FIXED_QUEST_SET', '0')
    );
    
    $ilDB->manipulateF(
        "UPDATE tst_tests SET question_set_type = %s WHERE random_test = %s",
        array('text', 'text'),
        array('RANDOM_QUEST_SET', '1')
    );
...
while ($row = $ilDB->fetchAssoc($set)) {
    $ilDB->queryF(
        "DELETE FROM cmi_custom WHERE lvalue = %s AND obj_id = %s AND user_id = %s",
        array('text','integer','integer'),
        array($lval,$row["obj_id"],$row["user_id"])
    );
}
?>
<#4080>
...
while ($row = $ilDB->fetchAssoc($set)) {
    $ilDB->queryF(
        "DELETE FROM scorm_tracking WHERE lvalue = %s AND obj_id = %s AND user_id = %s",
        array('text','integer','integer'),
        array($lval,$row["obj_id"],$row["user_id"])
    );
}
