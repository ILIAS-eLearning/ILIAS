...
class ilSetupPasswordManager
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!($this->getEncoderFactory() instanceof \ilSetupPasswordEncoderFactory)) {
            throw new \ilUserException(sprintf(
                '"encoder_factory" must be instance of \ilSetupPasswordEncoderFactory and set in %s.',
                json_encode($config)
            ));
        }
    }
     ...
     */
    public function setEncoderFactory(\ilSetupPasswordEncoderFactory $encoderFactory) : void
    {
